...
const AudioMonitor = ({
  ...
  const monitoringIntervalRef = useRef<null | NodeJS.Timeout>(null);
  ...
  const initAssistantSpeechMonitoring = () => {
    try {
      ...
      let wasSpeaking: boolean | false = false;
      ...
      monitoringIntervalRef.current = setInterval(() => {
        ...
        let sum = 0;
        const speechBandStart = Math.floor(
          300 /
            (audioContextRef.current.sampleRate /
              analyserRef.current.frequencyBinCount)
        );
        const speechBandEnd = Math.floor(
          3400 /
            (audioContextRef.current.sampleRate /
              analyserRef.current.frequencyBinCount)
        );
        ...
        for (
          let i = speechBandStart;
          i < Math.min(speechBandEnd, dataArray.length);
          i++
        ) {
          sum += dataArray[i];
        }
