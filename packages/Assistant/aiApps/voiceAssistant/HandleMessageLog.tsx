...
export const OutputMessageLog = () => {
  ...
  let messages = [
    ...masks.itemArray.map((item: any) => {
      return masks.chatMessages[item];
    }),
  ];
  console.log(
    messages,
    messages.filter((messages) => messages)
  );
...
export const HandleEventMessage = (
  ...
  switch (event.type) {
    case "conversation.item.input_audio_transcription.completed": {
      ...
      globalThis?.SetAiTextMessages &&
        globalThis.SetAiTextMessages([...OutputMessageLog()]);
    ...
    case "response.output_audio_transcript.delta": {
      setTagMask(
        ...
        "chatMessages",
        {
          ...masks.chatMessages,
          [`${event.item_id}`]: {
            message: masks.chatMessages[event.item_id]
              ? masks.chatMessages[event.item_id].message + event.delta
              : event.delta,
            role: "assistant",
          },
        },
