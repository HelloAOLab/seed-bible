...
if (controlBot.tags.parentBotId) {
  ...
  const childrensId = [
    controlBot.tags.id,
    ...getAllChildIds(controlBot.tags.id),
  ];
  ...
  childIds.forEach((childId) => {
    ...
    if (childBot.tags.parentBotId === controlBot.tags.id) {
      ...
      const childrensId = [
        childBot.tags.id,
        ...getAllChildIds(childBot.tags.id),
      ];
  ...
  !controlBot.tags.parentBotId &&
  controlBot.masks.lineTo.length === 0
) {
  whisper(typingTool, "makeTextBox", {
    x: controlBot.tags[dim + "X"],
    y: controlBot.tags[dim + "Y"],
    label: controlBot.masks.label,
    config: {
      formAddress: controlBot.masks.formAddress
        ? controlBot.masks.formAddress
        : controlBot.tags.formAddress
          ? controlBot.tags.formAddress
          : null,
      voiceNote: controlBot.masks.voiceNote
        ? controlBot.masks.voiceNote
        : controlBot.tags.voiceNote
          ? controlBot.tags.voiceNote
          : null,
      prevScaleX: controlBot.tags?.prevScaleX
        ? controlBot.tags.prevScaleX
        : null,
      prevScaleY: controlBot.tags?.prevScaleY
        ? controlBot.tags.prevScaleY
        : null,
    },
  });
  destroy(controlBot);
