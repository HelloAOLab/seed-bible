...
import { getBookmarks } from "ext_discover.helper.getBookmarks";
import { getPlaylistContextMenuOptions } from "ext_discover.helper.getPlaylistContextMenuOptions";
import { getPlaylists } from "ext_discover.helper.getPlaylists";
import { openOnAddDateModal } from "ext_discover.helper.openOnAddDateModal";
import { openSelf } from "ext_discover.helper.openSelf";
import { openSharePlaylistModal } from "ext_discover.helper.openSharePlaylistModal";
import { registerDragDrop } from "ext_discover.helper.registerDragDrop";
import { registerGetLabel } from "ext_discover.helper.registerGetLabel";
import { registerShowPersonVideoOverlayGlobals } from "ext_discover.helper.registerShowPersonVideoOverlay";
import { showPlaylistInfo } from "ext_discover.helper.showPlaylistInfo";
import { tryAddDataToHistory } from "ext_discover.helper.tryAddDataToHistory";
import { tryInitPlaylistMaker } from "ext_discover.helper.tryInitPlaylistMaker";
import { defineStorageGlobals } from "ext_discover.storage.defineStorageGlobals";
import { getBookmarksManager } from "ext_discover.managers.BookmarksManager";
import { Overlay } from "ext_discover.components.Overlay";
...
export async function definePlaylistGlobals() {
  try {
    ...
    registerDragDrop();
    registerShowPersonVideoOverlayGlobals();
    ...
    playlistBot.OpenSelf = openSelf;
    playlistBot.tryInitPlaylistMaker = tryInitPlaylistMaker;
    playlistBot.tryAddDataToHistory = tryAddDataToHistory;
    playlistBot.SharePlaylistModal = openSharePlaylistModal;
    playlistBot.getPlaylists = getPlaylists;
    playlistBot.options = getPlaylistContextMenuOptions;
    playlistBot.ControlButtons = createControlButtons;
    playlistBot.onAddDate = openOnAddDateModal;
    playlistBot.showInfo = showPlaylistInfo;
    registerGetLabel();

    G.Playlist = playlistBot;
    G.Overlay = Overlay;
    getBookmarksManager();
    ...
    function findNameRank(
      ...
      playlistBot.tags.bibleArrangementsArray[0].forEach((testament: any) => {
  ...
  `;
    ...
    console.log("Error in definePlaylistGlobals.tsx", err);
  }
}
