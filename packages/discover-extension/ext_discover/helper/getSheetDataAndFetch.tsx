export async function getSheetDataAndFetch(that: any, thisBot: any) {
  try {
    ...
    function isValidNumber(str: string, giveBool = false) {
    ...
    const ranksOfBooks = G.findNameRank("", "", true);

    const sectionRanks = G.getSectionRanking();

    let { status, data, statusText } = await web.get(linkToHit).catch();
    if (status !== 200)
      return info(
    ...
    const heading = options
      .split(",")
      .map((head: string) => head.toLocaleLowerCase());
    ...
    dataFinal.forEach((data: any) => {
      const testamentKey = heading.find((ele: string) =>
        ele.startsWith("test")
      );
      const sectionKey = heading.find((ele: string) => ele.startsWith("sec"));
      const chapterKey = heading.find((ele: string) => ele.startsWith("chap"));
      const verseKey = heading.find((ele: string) => ele.startsWith("ver"));
      const bookKey = heading.find((ele: string) => ele.startsWith("book"));
      let dataType: any = "";

      const bookOrgKey = data[bookKey].trim();

      const booksMap: any = thisBot.tags.abbrevations;
      ...
      if (data[verseKey]) {
        ...
        if (verse.includes("-")) {
          ...
          if (isValidNumber(startIdx, true) && isValidNumber(endIdx, true)) {
            const start: any = isValidNumber(startIdx);
        ...
        if (chapter.includes("-")) {
          ...
          if (isValidNumber(startIdx, true) && isValidNumber(endIdx, true)) {
            ...
            for (let i = start; i <= end; i++) {
              ...
              const startChapter: any = bookData.startChapter || 1;
              const bookDetails = findNameRank(bookData.commonName);
