export function groupVerse(that?: any) {
  // type
  // content
  ...
  const checkAndAddData = (data = undefined) => {
    if (lastGroup.length > 0) {
      ...
      convertedData.push({
        ...
        id: `${G.pseudoIndentifier}${lastGroup[0].id}`,
        layers: lastGroup[0].layers,
      });
      lastGroup = [];
    } else {
      ...
      if (lastItem) {
        switch (dataCopy.type) {
          case "verse":
            ...
            const grpId = dataCopy.additionalInfo.groupID || "-2";
            const grpId2 = lastItem.additionalInfo.groupID || "-1";
