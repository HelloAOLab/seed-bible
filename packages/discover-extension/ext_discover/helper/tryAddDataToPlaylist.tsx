import { showNotificationPlaylistAdd } from "ext_discover.helper.showNotificationPlaylistAdd";

export function tryAddDataToPlaylist(that?: any) {
    ...
    idsActive.forEach((id) => {
      if (G[`${id}creatingPlaylist`] || that.force) {
        if (G[`${id}AddDataToPlaylist`]) {
          ...
          showNotificationPlaylistAdd({ items: dataItem });
        } else {
