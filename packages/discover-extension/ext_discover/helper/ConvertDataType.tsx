export function ConvertDataType(that?: any) {
  const allItems: any[] = [];
  let badData = 0;
  ...
  that.results?.forEach((item: any) => {
    ...
    if (
      ...
      switch (itemType) {
        case "youtube":
          {
            const { isValid, type, videoId } = G.validateUrl(item.link);
            allItems.push({
              additionalInfo: {
                isValid,
                link: item.link,
        ...
        case "iframe":
          {
            const { isValid, type } = G.validateUrl(item.link);
            allItems.push({
              additionalInfo: {
                isValid,
                link: item.link,
