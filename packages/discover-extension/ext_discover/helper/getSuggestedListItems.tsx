import { checkAndGetlinkData } from "ext_discover.helper.checkAndGetlinkData";

export function getSuggestedListItems(that?: any) {
  const results = checkAndGetlinkData({ fileName: that.searchText });
  ...
  results.forEach((item: any) => {
    ...
    if (itemType === "verse") {
      ...
      item.verse = item.verse.filter(
        (ele: number) => ele <= totalVerseInChapter
      );
