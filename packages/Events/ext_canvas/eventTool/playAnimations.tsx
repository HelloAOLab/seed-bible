const setToInit = (initConfig: any, selectedBot) => {
  ...
  setTagMask(
    selectedBot,
    `${[dim + "RotationX"]}`,
    Math.PI * 2 * initConfig.rX
  );
  setTagMask(
    selectedBot,
    `${[dim + "RotationY"]}`,
    Math.PI * 2 * initConfig.rY
  );
  setTagMask(
    selectedBot,
    `${[dim + "RotationZ"]}`,
    Math.PI * 2 * initConfig.rZ
  );
  setTagMask(selectedBot, `scale`, initConfig.scale);
};

const animateBot = async (selectedBot: any, frame) => {
  const dim = os.getCurrentDimension();
  if (frame?.type === "recording") {
    ...
    try {
      if (recording) {
        ...
        let interval = setInterval(() => {
          ...
          let state = states.find((s: any) => s.time >= currentTime);
          if (state) {
            hadState = true;
    ...
    frame.animation &&
      os.startFormAnimation(selectedBot, frame.animation, {
        loop: {
          mode: "repeat",
        },
      });
    ...
    if (frame.duration > 0) {
      await animateTag(selectedBot, {
        ...
        toValue: {
          ...
          [dim + "RotationX"]:
            Math.PI * 2 * frame.initPos.rX + Math.PI * 2 * frame.rX,
          [dim + "RotationY"]:
            Math.PI * 2 * frame.initPos.rY + Math.PI * 2 * frame.rY,
          [dim + "RotationZ"]:
            Math.PI * 2 * frame.initPos.rZ + Math.PI * 2 * frame.rZ,
          scale: frame.scale,
        },
      ...
      selectedBot.tags[dim + "RotationX"] =
        Math.PI * 2 * frame.initPos.rX + Math.PI * 2 * frame.rX;
      selectedBot.tags[dim + "RotationY"] =
        Math.PI * 2 * frame.initPos.rY + Math.PI * 2 * frame.rY;
      selectedBot.tags[dim + "RotationZ"] =
        Math.PI * 2 * frame.initPos.rZ + Math.PI * 2 * frame.rZ;
...
const playAll = async (selectedBot: any, botFrames: any, repeat) => {
