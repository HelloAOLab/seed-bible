...
const opacityCalc = (botPosition: any, parentPosition: any, initialOpacity) => {
...
const timeLineAndEventRetriver = (eventData: any) => {
  const timeLineEvents = {};
  for (let i = 0; i < eventData.length; i++) {
    if (timeLineEvents[eventData[i].startDate]) {
      timeLineEvents[eventData[i].startDate] = [
        ...timeLineEvents[eventData[i].startDate],
        eventData[i],
      ];
    } else {
  ...
  for (let key of timeLineEventKeys) {
    timeLine.push({
      ...
      events: timeLineEvents[key]
        ...
        .sort((a: any, b: any) => a.eventData.sortKey - b.eventData.sortKey),
    });
  }
...
const timeLineEvents = [
  ...timeLineAndEventRetriver(controlBot.tags.eventBotData.eventsHere),
];
