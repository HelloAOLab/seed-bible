function generateQuery(params: any) {
  let queryArray = [];
...
function attachQueryToURL(url: any, params) {
  ...
  if (customMask) {
    if (botById.masks[customMask] && botById.masks[customMask].length > 0) {
      for (let i = 0; i < botById.masks[customMask].length; i++) {
        childrenIds = [
          ...childrenIds,
          botById.masks[customMask][i],
          ...getAllChildIds(botById.masks[customMask][i], customMask),
        ];
      }
    } else {
      return [];
    ...
    if (botById.tags.lineTo && botById.tags.lineTo.length > 0) {
      for (let i = 0; i < botById.tags.lineTo.length; i++) {
        childrenIds = [
          ...childrenIds,
          botById.tags.lineTo[i],
          ...getAllChildIds(botById.tags.lineTo[i]),
        ];
      }
    } else {
      return [];
...
const parentCheck = (childId: any, parentId: any) => {
