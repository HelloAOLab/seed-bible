...
import { extractExplicitRefs } from "./explicit";

/** The CasualOS gateway; its `client` exposes the server-side `aiChat` procedure. */
type Os = SeedBibleState["os"];
...
async function askAi(
  ...
  // `ai.chat` was an AUX-runtime global that doesn't exist in the standalone
  // extension. Go through the records client's server-side `aiChat` procedure
  // instead, which keeps the provider key on the server.
  const res = await os.client.aiChat({
    messages: [
  ...
  if (!res.success) {
    console.warn(`aiChat failed: ${res.errorCode} — ${res.errorMessage}`);
    return [];
  }
  const content = res.choices[0]?.content;
  const text =
...
export async function inferRefs(
  ...
  const aiSegs = await askAi(os, input);
