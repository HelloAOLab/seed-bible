...
const dumbTabs = BibleVizUtils.Data.tags.dumbUserPresenceData.map((data) => {
  return data.tab;
});
const allTabs = [...tabsContext.tabs, ...dumbTabs];

const tabsPathMap = new Map(
  allTabs.map((tab) => {
    let { book, chapter } = tab.data;

    if (book === "Psalms")
      ({ book, chapter } = thisBot.ConvertCompletePsalmsToDivided({ chapter }));

    const { arrangementIndex, testamentIndex, sectionIndex } =
      thisBot.GetBookInfoPathByName({ name: book, arrangementIndex: 0 });
    const testamentName =
      BibleVizUtils.Data.vars.fixedArrangementsInfo[arrangementIndex]
        .testaments[testamentIndex].name;
    const sectionName =
      BibleVizUtils.Data.vars.fixedArrangementsInfo[arrangementIndex]
        .testaments[testamentIndex].sections[sectionIndex].name;
    const path = [
      new PieceInfo({
