...
if (bot.masks.transformer || bot.tags.transformer) {
  if (!bot.links.transformerLink)
    setTagMask(
      bot,
      "transformerLink",
      `🔗${bot.masks.transformer ?? bot.tags.transformer}`
    );
  const transformerScale =
    bot.links.transformerLink.masks.scale ??
    bot.links.transformerLink.tags.scale ??
    1;
  const transformerScales = BibleVizUtils.Functions.GetBotScales(
    bot.links.transformerLink
  );
