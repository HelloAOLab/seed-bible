...
if (data) {
  if (data.highlightColor != BibleVizUtils.Data.tags.highlightColor) {
    if (!BibleVizUtils.Data.masks.isInHistoryMode)
      setTagMask(data.piece, "color", BibleVizUtils.Data.tags.highlightColor);
  ...
  if (piece.masks.chapterOrigin == "layout") {
    chapterData = scriptureMap3DManagerManager.GetChapterDataById({
      id: piece.masks.layoutChapterDataId,
    });
  } else if (piece.masks.chapterOrigin == "stack") {
    chapterData = BibleStackManager.GetChapterDataById({
      id: piece.masks.stackChapterDataId,
    });
  }
  console.log({ chapterData: { ...chapterData }, piece: { ...piece } });
  const currentHighlightInfo = chapterData.GetHighlightInfoByKey(
    piece.masks.chunkPath ?? piece.masks.versePath
  );

  if (currentHighlightInfo) {
    if (currentHighlightInfo.color != BibleVizUtils.Data.tags.highlightColor) {
      if (!BibleVizUtils.Data.masks.isInHistoryMode)
        setTagMask(piece, "color", BibleVizUtils.Data.tags.highlightColor);
