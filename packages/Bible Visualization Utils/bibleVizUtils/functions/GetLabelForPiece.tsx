...
const { scaleY } = BibleVizUtils.Functions.GetDialogBotScaleY({
  scaleXLimit: 5,
  line: label,
  paddingX: 0.4,
  paddingY: 0.4,
...
const closestFormAddressAspectRatio = BibleVizUtils.Functions.ClosestNumber({
  arr: BibleVizUtils.Data.tags.dialogBoxFormAddresses.map(
    (formAddressesInfo: any) => {
      return formAddressesInfo.aspectRatio;
    }
  ),
  input: infoLabelAspectRatio,
});
const infoLabelFormAddress =
  BibleVizUtils.Data.tags.dialogBoxFormAddresses.find(
    (formAddressesInfo: any) => {
...
const infoLabelDate = date
  ? ObjectPooler.GetObjectFromPool({
      tag: BibleVizUtils.Data.tags.ObjectPoolTags.InfoLabelDate,
    })
  : null;
...
let infoLabelTailDesiredScales = {
...
const infoLabelDateDesiredScales = infoLabelDate && {
  x:
    BibleVizUtils.Functions.GetCurrentLabelDateFormat() ===
    BibleVizUtils.Data.tags.LabelDateFormats.Relative
...
const infoLabelDateScales = infoLabelDate
  ? BibleVizUtils.Functions.GetBotScales(infoLabelDate)
  : null;
  ...
  case BibleVizUtils.Data.tags.LabelPositioning.Top:
    {
      ...
      infoLabelOffset = new Vector3(
        0,
        groundedPieceLabelOffsetY + infoLabelScales.y / 2,
        5
      );
  ...
  case BibleVizUtils.Data.tags.LabelPositioning.RightSidedCorner:
    {
      ...
      infoLabelTailDesiredScales = {
        x: 0.7 / infoLabelTransformerDesiredScales.x,
        y: 0.7 / infoLabelTransformerDesiredScales.y,
        z: 0.7 / infoLabelTransformerDesiredScales.z,
      };
      infoLabelOffset = new Vector3(
        radialVector.length() + infoLabelScales.x / 2,
        0.5,
        5
      );
...
const infoLabelDateOffset = infoLabelDate
  ? new Vector3(
      infoLabelOffset.x +
        infoLabelScales.x / 2 / infoLabelTransformerDesiredScales.x -
        (BibleVizUtils.Functions.GetCurrentLabelDateFormat() ===
