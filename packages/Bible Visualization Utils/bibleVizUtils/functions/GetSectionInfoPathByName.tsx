...
for (const currentTestamentIndex in BibleVizUtils.Data.vars
  ...
  if (
    testamentInfo.sections.some((sectionInfo: any) => {
      return sectionInfo.name === name;
    })
  ) {
    testamentIndex = currentTestamentIndex;
    sectionIndex = testamentInfo.sections.findIndex((sectionInfo: any) => {
      return sectionInfo.name === name;
    });
    found = true;
    break;
...
if (!found) {
  for (const currentArrangementIndex in BibleVizUtils.Data.vars
    .fixedArrangementsInfo) {
    if (currentArrangementIndex == initialArrangementIndex) continue;
    for (const currentTestamentIndex in BibleVizUtils.Data.vars
      .fixedArrangementsInfo[currentArrangementIndex].testaments) {
      const testamentInfo =
        BibleVizUtils.Data.vars.fixedArrangementsInfo[currentArrangementIndex]
          .testaments[currentTestamentIndex];
      if (
        testamentInfo.sections.some((sectionInfo: any) => {
          return sectionInfo.name === name;
        })
