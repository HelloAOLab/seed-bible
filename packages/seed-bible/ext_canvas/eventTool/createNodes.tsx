...
const createSoureBot = async () => {
    ...
    if(uidData[0].type === "verse"){
        uidData = uidData.sort((a,b) => parseInt(a.verseNum) - parseInt(b.verseNum))
    }
...
const getGradientColor = (value) => {
    ...
    if (value === 0) {
        ...
        const red = Math.round((greenRGB[0] * value) / 30) + Math.round((blueRGB[0] * (30 - value)) / 30);
        const green = Math.round((greenRGB[1] * value) / 30) + Math.round((blueRGB[1] * (30 - value)) / 30);
        const blue = Math.round((greenRGB[2] * value) / 30) + Math.round((blueRGB[2] * (30 - value)) / 30);
