...
export const TestamentContent = ({hidden}) => {
    ...
    const { mapBooksInfo } = useMemo(() => {
        return thisBot.GetMapStructure({arrangementIndex: arrangementIndexRef.current, arrangement: arrangementRef.current});
    }, [])
    const reversedSections = useMemo(() => {
        return testament.sections.toReversed()
    })
    const [sectionsShownArray, setSectionsShownArray] = useState(reversedSections.map((_, index) => {return true}))
        ...
        <div className={`testamentContent ${hidden ? "hidden" : ""}`}>
            {reversedSections.flatMap((section, sectionIndex) => {
                ...
                if(!sectionLevelsColorsMapRef.current.has(levelColorsKey))
                {
                    ...
                    sectionLevelsColorsMapRef.current.set(levelColorsKey, sectionLevelsColors);
                }
                const isLastSectionInTestament = sectionIndex == (reversedSections.length - 1)
                ...
                return section.books.toReversed().map((_, bookIndex) => {
                    const mapPanelBookInfo = mapBooksInfo.find((info) => {
                        return info.arrangementIndex == arrangementIndexRef.current &&
                            info.testamentIndex == testamentIndex &&
                                info.sectionIndex == sectionIndex &&
                                    info.bookIndex == bookIndex
                    })
                    
                    const isFirstBookInSection = useMemo(() => { return bookIndex == 0 }, [])


                    const color = mapPanelBookInfo.color ?? sectionLevelsColorsMapRef.current.get(levelColorsKey).toReversed()[bookIndex];

                    return <Book 
                        hidden={!isFirstBookInSection && !sectionsShownArray[sectionIndex]}
