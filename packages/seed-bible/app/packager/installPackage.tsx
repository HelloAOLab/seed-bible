...
await(async function mainInstaller(that) {
    ...
    async function SetUpApplication(applicationFunction, bot, toolbarConfig) {
        function generateAppItem({ icon, iconUrl, label, AppComponent }) {
            ...
            const onHold = async () => {
                ...
                globalThis.SetElement({
                    App: !iconUrl ? <span className="material-symbols-outlined">{icon}</span> : <img
                        src={iconUrl}
                        style={{
                            width: "40px",
                            height: "42px",
        ...
        const toolbarOption = generateAppItem({ icon: toolbarConfig.icon, label: toolbarConfig.label, AppComponent: App, iconUrl: toolbarConfig?.iconUrl })
    ...
    async function SetUpApplicationWithoutApp(toolbarConfig, bot) {
        const toolbarOption = {
            ...
            isImg: toolbarConfig?.iconUrl ? true : false
        }
