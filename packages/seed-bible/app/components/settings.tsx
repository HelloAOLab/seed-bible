...
const SettingsSidebar = () => {
    ...
    const baseSettingsConfig = [
        ...
        { key: 'Extensions', label: 'Extensions', icon: 'extension', style: '', expandable: false, onClick: () => setSideBarMode('extensions') },
        ...
        { key: 'divider1', type: 'divider' },
        {
            key: 'pageSettings', label: 'Page Settings', icon: 'article', expandable: true, subItems: [
                ...
                { key: 'instant_mix', label: 'Editor', icon: `instant_mix`, onClick: () => setSideBarMode('editorToolbarSettings') },
    ...
    const generalSettingsConfig = [
        ...
        // Account row
        {
            ...
            onClick: () => { globalThis.AccountSettingsEnteredFrom = 'settings'; setSideBarMode('createAccountSettings') }
        },

        // Disabled rows
        { key: 'billing', label: 'Billing & services', icon: 'rule_settings', style: 'disabled' },
    ...
    const generalConfig = generalSettingsConfig.map(item => {
        if (item.type === 'divider') return item;
            ...
            {activeTab === 'space' ? <div className="settings-content">
                {spaceContentVisibility.spaceIcon !== false && (
                    <div className={`space-content-item ${spaceContentVisibility.spaceIcon === false ? 'hidden-item' : ''}`}>
                        ...
                        <div className="space-details">
                            <div {...eventHandlers} onClick={(e) => {
                                if (shouldSuppressClick())
                                    return

                                openPopupSettings(<SpaceSelector activeSpace={activeSpace} spaces={spaces} updateSpace={updateSpace} />, null, true)
                ...
                <div className="settings-list">
                    {settingsSections.map(({ divider, items }) => {
                        ...
                        return (
                            <div key={(divider && divider.key) || items.map(i => i.key).join('_')}>
                                {divider && (
                                    <div className="settings-divider">
                                        <div className="sidebarLine"></div>
                                    </div>
                                )}
                                {items.map(({ key, label, icon, expandable, subItems, type, onClick, style, hidden }) => (
                                    type === 'divider' ? null : (
                                        <div key={key} className={`settings-item-container ${hidden ? 'hidden-item' : ''}`}>
                                                ...
                                                >
                                                    ...
                                                    <div className="item-text">
                                                        {editMode && editingLabel === key ? (
                                                            ...
                                                            <span
                                                                onClick={editMode ? (e) => { e.stopPropagation(); startEditingLabel(key); } : undefined}
                                            ...
                                            {expandable && expandedSections[key] && subItems?.length > 0 && (
                                                        ...
                                                        <div key={subKey} className={`settings-item-container ${subHidden ? 'hidden-item' : ''}`}>
                                                            <div className="settings-item-wrapper">
                                                                {editMode && (
                                                                    <button
                                                                        className="hide-button"
                                                                        onClick={(e) => { e.stopPropagation(); toggleVisibility(subKey, key); }}
                                                                ...
                                                                <div onClick={onClick} className={`settings-item sub-item ${subHidden ? 'hidden' : ''}`}>
                                                                    <div className="item-icon">
                                                                    ...
                                                                    <div className="item-text">
                                                                        {editMode && editingLabel === `${key}.${subKey}` ? (
                                                                            ...
                                                                            <span
                                                                                onClick={editMode ? (e) => { e.stopPropagation(); startEditingLabel(subKey, key); } : undefined}
                                                                                className={editMode ? 'editable-label' : ''}
                                                                            >
                                                                                {subLabel}
                                                                            </span>
                ...
                activeTab === 'general' ? <div className="settings-content">
                    ...
                    {generalSections.map(({ divider, items }) => {
                        ...
                        const hasVisibleItem = editMode ? items.length > 0 : items.some(i => !i.hidden);
                            ...
                            <div key={(divider && divider.key) || items.map(i => i.key).join('_')}>
                                ...
                                {items.map((item) => {
                                    ...
                                    if (item.type === 'account') {
                                        ...
                                        return (
                                            <div key={item.key} className={`activeAccount ${hidden ? 'hidden-item' : ''}`} style={{ position: 'relative' }}>
                                                {editMode && (
                                                    <button
                                                        className="hide-button"
                                                ...
                                                {userData && userData?.photoLink ? <img style={{ 'borderRadius': '50%', height: '40px', width: '40px', border: '1px solid #4459F3' }} src={userData.photoLink} /> : <UserAvatar />}
                                                <div className="softText">
                                                    {editMode && editingLabel === item.key ? (
                                                        <input
                                                            type="text"
                                                            value={label}
                                                            onChange={(e) => handleLabelEdit(item.key, '', e.target.value)}
                                                            onBlur={finishEditingLabel}
                                                            onKeyPress={(e) => e.key === 'Enter' && finishEditingLabel()}
                                                            className="label-edit-input"
                                                            autoFocus
                                                        ...
                                                        <span
                                                            onClick={editMode ? () => startEditingLabel(item.key) : undefined}
                                                            className={editMode ? 'editable-label' : ''}
                                                        >
                                                            {label}
                                                        </span>
                                                ...
                                                {!userData && (
                                                    <div style={{ "justifyContent": "center" }} className="activeAccount">
                                                        <button onClick={() => { globalThis.AccountSettingsEnteredFrom = 'settings'; setSideBarMode('createAccountSettings') }} className="create-profile-btn">
                                    ...
                                    if (item.type === 'header') {
                                        if (hidden && !editMode) return null;
                                        return (
                                            <div key={item.key} className={`space-details ${hidden ? 'hidden-item' : ''}`} style={{ position: 'relative' }}>
                                                {editMode && (
                                                    <button
                                                        className="hide-button"
                                                        onClick={() => toggleVisibility(item.key)}
                                    ...
                                    if (item.type === 'desc') {
                                        if (hidden && !editMode) return null;
                                        return (
                                            <div key={item.key} className={`space-description ${hidden ? 'hidden-item' : ''}`} style={{ position: 'relative' }}>
                                                {editMode && (
                                                    <button
                                                        className="hide-button"
                                                        onClick={() => toggleVisibility(item.key)}
                                    ...
                                    if (item.type === 'section' && item.key === 'subscriptions') {
                                        if (hidden && !editMode) return null;
                                        return (
                                            <div key={item.key} className={`general-section ${hidden ? 'hidden-item' : ''}`} style={{ position: 'relative' }}>
                                                {editMode && (
                                                    <button
                                                        className="hide-button"
                                                        onClick={() => toggleVisibility(item.key)}
                                                ...
                                                <div style={{ justifyContent: 'center' }} className="activeAccount">
                                                    {!subscribe ? (
                                                        <button onClick={() => setSubscribe(true)} className="create-profile-btn">
                                                            Subscribe
                                                        </button>
                                                    ) : (
                                                        <div style={{ position: '' }}>
                                                            ...
                                                            >
                                                                ...
                                                                />
                                                                <button onClick={() => search()} style={{ borderRadius: '8px' }} className="create-profile-btn">
                                                                    <span className="material-symbols-outlined">person_add</span>
                                                                </button>
                                                            </div>

                                                            {searchResult && (
                                                                <div className="profileCard" style={{ position: 'absolute', left: '300px', top: '35%' }}>
                                    ...
                                    if (hidden && !editMode) return null;
                                    return (
                                        <div key={item.key} className={`settings-item-container ${hidden ? 'hidden-item' : ''}`}>
                                            <div className="settings-item-wrapper">
                                                {editMode && (
                                                    <button
                                                        className="hide-button"
                                                ...
                                                <div
                                                    ...
                                                    className={`settings-item ${item.style || ''} ${hidden ? 'hidden' : ''}`}
                                                >
                                                    <div className="item-icon">
                                                        <span className="material-symbols-outlined">{item.icon}</span>
                                                    </div>
                                                    <div className={`item-text ${item.style === 'disabled' ? 'disabled' : ''}`}>
                                                        {editMode && editingLabel === item.key ? (
                                                            <input
                                                                type="text"
                                                                value={label}
                                                            ...
                                                            <span
                                                                onClick={editMode ? (e) => { e.stopPropagation(); startEditingLabel(item.key); } : undefined}
                                                                className={editMode ? 'editable-label' : ''}
                                                            >
                                                                {label}
                                                            </span>
