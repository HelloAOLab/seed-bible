...
const ToolbarSettings = () => {
  ...
  const toggleToolActive = (index: any, checked: any) => {
    os.log(checked);
    if (checked === undefined) checked = true;
    if (currentMode === "Page") {
      setTools(
        tools.map((tool, i: any) =>
          i === index ? { ...tool, active: checked } : tool
        )
      );
    } else if (currentMode === "Canvas") {
      setCanvasTools(
        canvasTools.map((tool: any, i: any) =>
          i === index ? { ...tool, active: checked } : tool
        )
      );
    } else if (currentMode === "Map") {
      setMapTools(
        mapTools.map((tool: any, i: any) =>
          i === index ? { ...tool, active: checked } : tool
        )
      );
    }
            ...
            >
              <span className="icon">
                {typeof tool.icon === "string" && !tool.isImg ? (
                  ...
                  <img src={tool.icon} style={{ width: "40px" }} />
                ) : (
                  tool.icon
                )}
              </span>
