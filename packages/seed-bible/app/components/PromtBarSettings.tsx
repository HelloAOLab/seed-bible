...
const PromtBarSettings = () => {
  ...
  const toggleToolActive = (index: any, checked: any, type: any) => {
    os.log(checked);
    if (checked === undefined) checked = true;
    if (type === "promt") {
      setPromtTools(
        promtTools.map((tool, i: any) =>
          i === index ? { ...tool, active: checked } : tool
        )
      );
    } else if (type === "mindmap") {
      setMindmapTools(
        mindmapTools.map((tool: any, i: any) =>
          i === index ? { ...tool, active: checked } : tool
        )
      );
    }
  ...
  return (
    <div className="aiSettings-container boundElements">
      ...
      {switcher === 1 && (
        <>
          <ul className="tool-list">
            {promtTools.map((tool: any, index: any) => {
              // os.log(tool)
              return (
                <li
                  key={index}
      ...
      {switcher === 2 && (
        <>
          <ul className="tool-list">
            {mindmapTools.map((tool: any, index: any) => {
              return (
                <li
                  key={index}
                  className={`tool-item ${tool.active ? "active" : ""}`}
