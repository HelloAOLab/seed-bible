...
export function exportTextConfigToCSS(textConfig: any) {
  const toCSSVarName = (section: any, key) => `--text-${section}-${key}`;
  const cssVars = [];
...
function TextSettings() {
  ...
  const handleStyleChange = (section: any, newConfig: any) => {
    const updatedConfig = {
      ...textConfig,
      [section]: newConfig,
    };
  ...
  useEffect(() => {
    setTextConfig(
      spaces.find((e: any) => e.id === activeSpace)?.settings?.text?.data ||
        defaultTextConfig
    );
  }, [activeSpace]);
  return (
    <div className="textSettings-sidebar">
      ...
      {[
        ...
        <div key={section} className="flexColumn-10">
          ...
          >
            <option
              selected={textConfig[section].font === `'Newsreader', serif`}
            >
              Newsreader
            </option>
            <option
              selected={
                textConfig[section].font === `'Plus Jakarta Sans', sans-serif`
              }
            >
              Plus Jakarta Sans
            </option>
          ...
          <TextFormattingToolbar
            ...
            onChange={(newConfig: any) => handleStyleChange(section, newConfig)}
          />
        </div>
      ))}
...
const TextFormattingToolbar = ({ sectionStyles, onChange }: any) => {
