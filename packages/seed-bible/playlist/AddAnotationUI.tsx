        ...
        >
                ...
                {checklistEnabled ?
                    <Checkbox
                        ...
                        onClick={() => {
                            ...
                            if (!embedding && data.type !== 'heading' && !isSomethingEmbededChecked) {
            ...
            <div className="actions">
                ...
                {data.type === "heading" ?
                    ...
                    : null}
                {embeded ?
                    <p className={`end-icon without-right-margin ${`${isMobile && "visible"} end-icon without-right-margin`}`} onClick={() => onDisembed({ idFinal: data.id, pId: pId })} >
                ...
                <p className={`end-icon without-right-margin ${`${isMobile && "visible"} end-icon without-right-margin`}`} onClick={() => deleteFromList(data.id)} >
                    <span class="material-symbols-outlined unfollow delete-icon">
                        delete
                    </span>
                </p>
        ...
        {!embeded && expand && <div style={{ paddingLeft: '1rem' }}>
            {data.additionalInfo.layers?.map((ele, indexInner) => (ele.type === "attachment-link" || ele.type === "date")
                ...
                <AttachmentLinkItem
                    ...
                    playlistName={false}
                    currentFormat={false}
                    checked={checkListEmbeded?.[data.id]}
        ...
        >
            ...
            {(isSomethingChecked || embedding) &&
                <div style={{ justifyContent: 'space-between', margin: '0.5rem 0' }} className="align-center">
                    ...
                    {(!!embedding && isSomethingChecked && !isSomethingEmbededChecked) && <Button
            ...
            {finalHistoryObject.length === 0 && <p style={{ margin: '1rem 0' }}>Add Items to start Annotating.</p>}
            {
                finalHistoryObject.map((ele, index) => (ele.type === "attachment-link" || ele.type === "date")
                    ...
                    /> : <>
                        ...
                        {!draggedItemID && selectedAnnotation === ele.id && !embedding && <div style={{ padding: '1rem' }}>
                            <AttachLink onAddTags={onAddTags} isTags attachLink={onEmbedItems} massAdd={massAdd} />
                        </div>}
                    </>)
            }

            {!selectedAnnotation && !draggedItemID && !embedding && <AttachLink attachLink={attachLink} massAdd={onMassAdd} />}
