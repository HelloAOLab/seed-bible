...
import { ProjectProvider } from "playlist.playlist.useProjectContext";
    ...
    useEffect(() => {
        ...
        const tt = setTimeout(() => {
            if (globalThis.hasASharedPlaylist) {
                const playlist = (globalThis[`${tab}playlists`] || []).find(ele => ele.id === globalThis.hasASharedPlaylist);
        ...
        apiCallforAnnotationRef.current = setTimeout(() => {
            ...
            (async () => {
                try {
                    ...
                    const promises = [];
                    res.forEach(ele => {
                        promises.push(web.get(ele.data.urlArray[0]));
                    });
                    ...
                    console.log(allAnnotations, res, "allAnnotations");
            ...
            <ProjectProvider>
                ...
                >
                    ...
                    >
                            ...
                            <div className={`playlist-cont-actions`} style={{ padding: !editData.id ? '' : '12px' }}>
                                ...
                                {!editData.id && <div className="tabs-playlist" style={{ width: '100%' }}>
                                    {buttonConfigs.map(({ label, onClick, value, icon }) => <h4
                                        onClick={() => {
                                            if (SplitAppPanel2) return ShowNotification({ message: 'Cannot Switch While Playing a Specific type!', severity: "error" });
                                            onClick();
                                        }}
                        ...
                        {isLayers ?
                            <div style={{ display: "flex", flexDirection: 'column', height: `calc(100% - ${playingPlaylist || !!editData.id ? '130px' : '40px'})` }}>
                                <Discover editingPlaylist={editData.id} currentOpenedBook={currentOpenedBook} fetchingAnnotation={fetchingAnnotation} chapter={currentOpenedBook?.chapter} annotationData={annoationData} style={{ height: `100%` }} setOpenModal={setOpenModal} playingPlaylist={playingPlaylist} />
                            </div>
                            :
                            <div style={{ display: "flex", overflow: 'scroll', height: `calc(100% - ${playingPlaylist || !!editData.id ? '90px' : '0px'})` }}>
                                <CreatePlaylistUI setTab={setTab} isCreate setOpenModal={setOpenModal} active={true} playingPlaylist={playingPlaylist} id='default' />
                            </div>
                        }
                    </div>
                </div>
