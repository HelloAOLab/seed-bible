...
const DragDrop = ({ massAdd, attachLink, setItemSelected, itemSelected, access, isSomethingEmbededChecked, checkListEmbeded, setChecklistEmbeded, onDisembed = () => { }, layers = true, embedding, setEmbedding, setRef = {}, allowHeadingCheck, selectedTags, playlistName, currentDateActive, clickPass, currentFormat, readingPlanEnabled, linkingMode, viewOnly, checkListData = {}, oldItemsMap = {}, parentId, list, isPlayer, playingPlaylist, activeItemList = {}, activeItemID, toggleRemoved, setList, editDataFromPlaylist, playListSubId, setPlaylistFromRow = () => { }, playListSubIndex = null, deleteFromList = () => { }, onClickItem = () => { }, onClick = () => { }, creatingPlaylist = false, color, icon, isCustomColor, description, isCustomIcon }) => {
    ...
    const transformedHistory = useMemo(() => layers ? thisBot.groupVerse(list) : thisBot.groupVerse(list), [list, selectedCount, unSelectedCount]);
    ...
    const handleDragStart = (index, pId) => {
        ...
        if (pId) {
            ...
            const itemId = transformedHistory[pIndex].additionalInfo.layers[index]?.id;
    ...
    const handleDragOver = (index, pseudoIndex = 1, pseudoID = null, event) => {
        ...
        if (draggedItemIndex === -1 && parentIdx > -1) {
            ...
            dragItem = [transformedHistory[parentIdx].additionalInfo.layers[draggedItemIndex]];
        }

        let draggedOverItem = transformedHistory[index];

        if (pseudoID) {
            const parentIndexDragOver = transformedHistory.findIndex(ele => ele.id === pseudoID);
        ...
        newItems = [...transformedHistory.filter((hist) => !filterAbleItems[hist.id])];
    ...
    const handleDragEnd = () => {
        ...
        const dragOverItem = transformedHistory.find(ele => ele.id === dragOverSet.itemId);
        ...
        {list.length === 0 && <h4 style={{ margin: "8px 0" }}>No Items Now.</h4>}
        {
            transformedHistory.map((data, index) => data.type?.includes("range") || (data.additionalInfo?.layers?.length > 0 && layers)
                ...
                <PlaylistContentRenderer
                    ...
                    isAdditionalInfo={data.type?.includes("range") && !data.additionalInfo?.layers?.length}
                    embedding={embedding}
                        ...
                        <>
                                ...
                                onPointerDown={() => {
                                    if (!viewOnly) {
                                        if (data.type === "heading" && !embedding) {
                                            ...
                                            if (!isMultiFunctionHold && !checklistEnabled) setItemSelected(prev => prev === data.id ? null : data.id);
                                        }
                                    }
                                    if (!viewOnly) {
                            ...
                            >
                                ...
                                <div className="start-actions">
                                    ...
                                    {(data.type !== "heading" || allowHeadingCheck) && checklistEnabled && !viewOnly ?
                                        <Checkbox
                                            ...
                                            checked={checkListData[data.id] || data.readAlready || embedding === data.id}
                                            onClick={() => {
                                ...
                                <p
                                    ...
                                    style={{ border: data.id === itemSelected ? '1px solid #D36433' : '', backgroundColor: data.id === itemSelected ? '#D364334D' : '' }}
                                ...
                                <div className="actions">
                                    {data.type === "heading" ?
                                        <p className={`end-icon without-right-margin ${`${isMobile && "visible"} end-icon without-right-margin`}`}
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                thisBot.EditRichText({ id: data.id, text: data.content })
                                            }} >
                                        ...
                                        : null}
                                    {!playingPlaylist && data.type !== 'heading' && !embedding && layers && creatingPlaylist && !viewOnly && <p className={`end-icon without-right-margin ${`${isMobile && "visible"} end-icon without-right-margin`}`}
                                        onClick={(e) => {
                                            e.stopPropagation();
                                    ...
                                    {!playingPlaylist && creatingPlaylist && !viewOnly && <p className={`end-icon without-right-margin ${`${isMobile && "visible"} end-icon without-right-margin`}`} onClick={(e) => { e.stopPropagation(); deleteFromList(index); }} >
                            ...
                            {itemSelected === data.id && !draggedItemID && !embedding && <div style={{ padding: '1rem' }}>
...
const PlaylistContentRenderer = ({
    ...
    return <div>
        ...
        >
            ...
            {!isAdditionalInfo && toBeMapArray.map((data) => <input
                style={{ opacity: '0', 'pointer-events': 'none', position: 'absolute', 'left': 0, 'top': 0, zIndex: -1 }}
                placeholder={'test'}
                ref={(ref) => {
                    if (setRef && setRef[data.id]) {
            ...
            <div className="actions">
                {!playingPlaylist && layers && !embedding && creatingPlaylist && !viewOnly && <p className={`end-icon without-right-margin ${`${isMobile && "visible"} end-icon without-right-margin`}`} onClick={(e) => { e.stopPropagation(); setEmbedding(data.id) }} >
                    <span class="material-symbols-outlined">
                        pip
                    </span>
                </p>
                ...
                {!isAdditionalInfo && toBeMapArray.length > 0 && <p className="without-right-margin end-icon visible">
                    <span onClick={() => {
                        if (!dragged.current) {
                            setOpen(p => !p);
                        }
        ...
        <div style={{ height: open ? "auto" : '0', transition: "all 0.2s linear", overflow: 'hidden', padding: "0 8px" }}>
            {!isAdditionalInfo && toBeMapArray.map((data, index) => {
                return (data.type === "attachment-link" || data.type === "date")
                    ...
                    >
                        ...
                        <div className="actions">
                            ...
                            {!playingPlaylist && creatingPlaylist && open && !viewOnly && <p className={`end-icon without-right-margin ${`${isMobile && "visible"} end-icon without-right-margin`}`} onClick={() => deleteFromList(index, id)} >
                                <span class="material-symbols-outlined unfollow delete-icon">
                                    delete
                                </span>
                            </p>}
