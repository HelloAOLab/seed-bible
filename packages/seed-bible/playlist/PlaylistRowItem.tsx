...
const startEditingPlaylist = (name, id, list, subId, attachment, checklistEnabled, parentId, readingPlanEnabled, currentFormat, color, icon, isCustomColor, description, isCustomIcon, selectedTags, isLayers, access) => {
...
const PlaylistRowItem = ({ currentDateActive, oldItemsMap = {}, checkListData, selectedPlaylists, selectPlaylist = false, setSelectPlaylist, playlistParentName = '', clickPass = false, linkingMode, onLink, viewOnly, parentId, playingPlaylist, checklistEnabled, readingPlanEnabled, totalItem, index, toggle, list, name, id, setPlaylists, attachment = null, playListIndex, playListSubId = null, playListSubIndex = null, creatingPlaylist, handleDragOver, handleDragEnd, currentFormat, handleDragStart, dragOverSet, setOpenedList, opendedList, color = "#D9D9D9", icon = "subscriptions", isCustomColor = false, description = '', isCustomIcon = false, selectedTags, isLayers, access }) => {
    ...
    const percentageCompleted = (() => {
        if (id) {
            ...
            const playlistList = (globalThis[`${id}playlists`] || []).find(ele => ele.id === id);
                    ...
                    >
                        {
                            ...
                            <>
                                ...
                                <div className="more-menu-items"
                                    onClick={() => {
                                        startEditingPlaylist(name, id, list, playListSubId, attachment, checklistEnabled, parentId, readingPlanEnabled, currentFormat, color, icon, isCustomColor, description, isCustomIcon, selectedTags, isLayers, access);
                                        setShowMoreOptions(false);
                                    }}
                                >
                                    <p>Edit Playlist</p>
