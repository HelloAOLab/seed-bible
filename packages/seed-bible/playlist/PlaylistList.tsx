...
const PlaylistList = ({ extraActions = () => { }, mergeMode, selectedPlaylists, setSelectPlaylist, selectPlaylist = false, playLists, setPlayLists, creatingPlaylist = false, playingPlaylist, parentId, isLayers }) => {
    ...
    return <>
        <div onClick={() => extraActions()} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {playLists.length === 0 && <h4 style={{ margin: "8px 0" }} >No {isLayers ? 'Layers' : 'Playlists'} to show.</h4>}
            {playLists.filter(pl => !playingPlaylist ? true : pl.id === playingPlaylist).map(({ access, name: playlistName, list, id, nesting, toggleRender, description, readingPlanEnabled, dateFormat, attachment, checklistEnabled, color, icon, isCustomColor, isCustomIcon, selectedTags, isLayers }, index) =>
                <PlaylistRowItem
