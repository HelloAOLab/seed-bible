...
switch (true) {
  case elementData instanceof MapBookData:
    {
      ...
      const mapBookStructure = thisBot.GetMapBookStructureByChild({
        mapBookData: elementData,
      });
  ...
  case elementData instanceof MapChapterData:
    {
      NullifyMapChapterParentIds(elementData, [
        nullifiableIds.mapId,
        nullifiableIds.mapBookId,
      ]);
...
async function CreateDataCopy(data: any) {
  ...
  switch (true) {
    case data instanceof MapBookData:
      {
        copy = await thisBot.CreateMapBook({
          bookInfo: data.elementInfo,
          mapData,
        });
      }
      break;
    case data instanceof MapChapterData:
      {
        copy = await thisBot.CreateMapChapter({
