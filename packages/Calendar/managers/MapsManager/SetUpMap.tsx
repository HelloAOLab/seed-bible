...
mapData.childrenStructures.forEach(async (mapBookStructure: any) => {
...
for (
  ...
  i <
  Math.min(
    MapElementMeasurements.MaxAmountOfColumns,
    mapData.childrenStructures.length
  );
  i++
) {
  const segment = {
    start:
      ...
      (MapElementMeasurements.BookScaleX +
        MapElementMeasurements.BookHorizontalGap) *
        i,
    end:
      booksOriginPosition.x +
      (MapElementMeasurements.BookScaleX +
...
for (let row = 0; row < mapData.amountOfRows; row++) {
  ...
  const bookStructuresWithinRow = mapData.childrenStructures.filter(
    (mapBookStructure: any) => {
      return mapBookStructure.row === row;
    }
  );

  bookStructuresWithinRow.forEach((mapBookStructure: any) => {
    if (mapBookStructure.mapBookData.element.tags.scaleY > greaterBookScaleY)
      greaterBookScaleY = mapBookStructure.mapBookData.element.tags.scaleY;

    const bookPosition = new Vector2(
      booksOriginPosition.x +
        mapBookStructure.mapBookData.element.tags.scaleX / 2 +
        (mapBookStructure.mapBookData.element.tags.scaleX +
          MapElementMeasurements.BookHorizontalGap) *
          mapBookStructure.column,
      currRowPosition -
        sectionLineLabelScaleY -
        sectionLineScaleY -
        MapElementMeasurements.BookVerticalGap -
        MapElementMeasurements.BookLabelHeight -
        mapBookStructure.mapBookData.element.tags.scaleY / 2
    );
    setTag(
    ...
    const sectionColor = InstanceManager.vars.fixedArrangementsInfo[
      ...
      mapBookStructure.mapBookData.creationInfo.sectionIndex
    ].color;
  ...
  rowSegment.end =
    ...
    greaterBookScaleY;
  currRowPosition +=
    rowSegment.end - rowSegment.start - MapElementMeasurements.BookVerticalGap;
...
const gridElementsOffset = new Vector3(
  -booksGridScales.x / 2,
  booksGridScales.y / 2,
  0
);
const bookGridElements = [
  ...mapData.childrenStructures.flatMap((mapBookStructure: any) => {
    return [
      mapBookStructure.mapBookData.element,
...
mapData.staticMapElements.settingsButtons.forEach(
      ...
      case MapButtonType.PlaylistSelectorButton:
        {
          ...
          const { scaleX: labelScaleX } = GetDialogBotScaleY(
            scaleXLimit,
            settingsButton.links.buttonLabel.tags.label,
            0,
            0
          );
...
mapData.childrenStructures.forEach((mapBookStructure: any, index: any) => {
