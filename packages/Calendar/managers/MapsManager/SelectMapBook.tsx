...
const chaptersOriginPosition = new Vector2(
  mapBookPosition.x - mapBookData.element.tags.scaleX / 2,
  mapBookPosition.y + mapBookData.element.tags.scaleY / 2
);
var column = 0;
var row = 0;
...
if (!fromOpenAllButton) {
  if (mapData?.isCameraAnimationEnabled) {
    os.focusOn(
      { x: mapBookPosition.x, y: mapBookPosition.y, z: 0 },
      {
...
const { relativeDateRange } =
  StacksManager.tags.booksStaticInfo[mapBookData.elementInfo.commonName];
...
for (let mapChapterData of mapBookData.childrenData) {
  const mapChapter = ObjectPooler.GetObjectFromPool({
  ...
  const mapChapterMod = {
    ...
    [dimension + "X"]:
      ...
      MapElementMeasurements.ChapterWidth / 2 +
      MapElementMeasurements.ChapterPadding +
      column *
        (MapElementMeasurements.ChapterWidth +
          MapElementMeasurements.ChapterGap),
    [dimension + "Y"]:
      chaptersOriginPosition.y -
      MapElementMeasurements.ChapterHeight / 2 -
      MapElementMeasurements.ChapterPadding -
      row *
    ...
    desiredLabel:
      mapBookData.element.tags.startChapter + mapChapterData.elementInfo.number,
  ...
  if (InstanceManager.masks.isInHistoryMode)
    setTagMask(mapChapter, "color", GetHistoryColor({ element: mapChapter }));
  else if (mapChapterData.highlightColor)
    setTagMask(chapter, "color", mapChapterData.highlightColor);
  column += 1;
...
if (fromOpenAllButton) {
  mapBookData.childrenData.forEach((mapChapterData: any) => {
    ...
    setTag(
      ...
      "label",
      mapChapterData.element.tags.desiredLabel
    );
  });
  return true;
} else {
  return Promise.all(
    mapBookData.childrenData.map((mapChapterData, index) => {
    ...
    shout("OnSelectMapBookComplete", { fromOpenAllButton });
  });
}
