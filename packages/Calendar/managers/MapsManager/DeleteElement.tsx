...
function DeleteMapChapter(mapChapterData: any) {
  ...
  if (mapChapterData.element) {
    InstanceManager.TryHideUsersNotificationOnElement({
      element: mapChapterData.element,
    });
    if (
      mapChapterData.isSelected &&
      ...
      mapChapterData.element.vars.chunksOfVerses.splice(
        0,
        mapChapterData.element.vars.chunksOfVerses.length
      );
    }
    ObjectPooler.ReleaseObject({
      obj: mapChapterData.element,
      tag: mapChapterData.element.tags.poolTag,
    });
...
function DeleteMapBook(mapBookData: any) {
  ...
  if (mapBookData.element) {
    ObjectPooler.ReleaseObject({
      obj: mapBookData.element,
      tag: mapBookData.element.tags.poolTag,
    });
...
function DeleteMap(mapData: any) {
  ...
  mapData.childrenStructures.forEach((mapBookStructure: any) => {
    ...
    ObjectPooler.ReleaseObject({
      obj: mapBookStructure.nameLabel,
      tag: mapBookStructure.nameLabel.tags.poolTag,
    });
    ObjectPooler.ReleaseObject({
      obj: mapBookStructure.dateLabel,
      tag: mapBookStructure.dateLabel.tags.poolTag,
    });

    mapBookStructure.mapBookData = null;
    mapBookStructure.nameLabel = null;
    mapBookStructure.dateLabel = null;
    const mapBookStructureIndex =
      thisBot.vars.mapBooksStructure.indexOf(mapBookStructure);
    if (mapBookStructureIndex >= 0)
      thisBot.vars.mapBooksStructure.splice(mapBookStructureIndex, 1);
  });
