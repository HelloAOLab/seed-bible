...
const { arrangementIndex, testamentIndex, sectionIndex, found } =
  BibleVizUtils.Functions.GetBookInfoPathByName({ name });
let book, bookData;
if (found) {
  const sectionInfo =
  ...
  const level = levels.find((level: any) => {
    return level.some((bookInfo: any) => {
      return bookInfo.commonName == name;
    });
  });
  const levelIndex = levels.indexOf(level);
  const bookInfo = sectionInfo.books.find((currentBookInfo: any) => {
  ...
  const amountOfChaptersInLevel = level.reduce((total: any, bookInfo: any) => {
  ...
  const sectionAvailableSpace =
    ...
    BibleVizUtils.Data.tags.StackSpacing.BetweenBooks * (levelsLenght + 1);
  ...
  if (bookData.pieceInfo.group) {
    ...
    ({ groupBookScaleX, groupBookScaleY } =
      BibleVizUtils.Functions.GetGroupBookData({ bookLayout }));
  }
  ...
  const levelsColors = levels.map((level: any, i: any) => {
    const levelColorRGB = [
  ...
  const bookMod = {
    ...
    hoveredScaleX:
      (groupBookScaleX ??
        BibleVizUtils.Data.tags.StackPieceMeasurements.BookScales.x) +
      BibleVizUtils.Data.tags.StackPieceMeasurements.AditionalBookScaleOnHover,
    hoveredScaleY:
      (groupBookScaleY ??
        BibleVizUtils.Data.tags.StackPieceMeasurements.BookScales.y) +
      BibleVizUtils.Data.tags.StackPieceMeasurements.AditionalBookScaleOnHover,
    ...
    singleBooksScales:
      BibleVizUtils.Data.tags.StackPieceMeasurements.BookScales,
    toErase: true,
  };
