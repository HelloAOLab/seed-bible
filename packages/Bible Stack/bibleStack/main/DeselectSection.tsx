...
const infoLabelTransformer =
  BibleVizUtils.Functions.GetCurrentInfoLabelTransformer(sectionData.shadow);
const selectedBooksData = sectionData.childrenData
  .flat()
  .filter((bookData: any) => {
...
if (thisBot.vars.highlightedPieces.length > 0) {
  const piecesToUnhighlight = sectionData.childrenData
    ...
    .filter((bookData: any) => {
      return (
        bookData.isActive &&
        thisBot.vars.highlightedPieces.some((piece: any) => {
...
if (selectedBooksData.length > 0) {
  selectedBooksData.forEach((bookData: any) => {
