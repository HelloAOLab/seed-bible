...
if (sectionData.isInExplodedView) {
  ...
  piecesAboveSection.forEach((piece) => {
    ...
    pieceNewPositionZ =
      ...
      deltaScaleZ +
      BibleVizUtils.Data.tags.StackSpacing.ExplodedViewSectionPadding * 2;
  ...
  if (isInstantaneous)
    ...
    await Promise.allSettled([
      ...firstSequenceAnimationsObjects.map((animateTagObject) => {
        return BibleVizUtils.Functions.GetAnimateTagFromObject({
          obj: animateTagObject,
        });
      }),
      os.focusOn(
...
for (const bookDataArr of sectionData.childrenData) {
  ...
  let levelScaleZ;
  const amountOfChaptersInLevel = bookDataArr.reduce(
    (total: any, bookData: any) => {
  ...
  for (const bookData of bookDataArr) {
    ...
    bookDesiredPositionZ = sectionData.isInExplodedView
      ? sectionData.piece.tags.desiredPositionZ +
        ...
        levelScaleZ / 2 +
        (sectionData.piece.masks.isOnTheGround
          ? BibleVizUtils.Data.tags.StackSpacing
    ...
    if (bookData.pieceInfo.group) {
      ...
      ({
        ...
        groupBookLayoutPositionX,
        groupBookLayoutPositionY,
      } = BibleVizUtils.Functions.GetGroupBookData({
        bookLayout,
        sectionPosition,
      }));
    }
  ...
  if (!sectionData.isInExplodedView) {
    bookDesiredPositionZOnRegularView +=
      levelScaleZ + BibleVizUtils.Data.tags.StackSpacing.BetweenBooks;
  }
}
