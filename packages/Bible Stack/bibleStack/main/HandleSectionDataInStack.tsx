...
if (sectionData.isSplitIntoBooks) {
  ...
  const sectionShadowDesiredPositionZ =
    ...
    activeBooksInsideSection.length > 0
      ? BibleVizUtils.Data.tags.StackSpacing.ExplodedViewSectionShadowPadding
      : 0);
  nextPositionZ += sectionData.piece.masks.isOnTheGround
    ? 0
    : sectionData.isInExplodedView
      ? BibleVizUtils.Data.tags.StackSpacing.ExplodedViewSectionPadding
  ...
  for (const bookDataArr of sectionData.childrenData) {
    for (const bookData of bookDataArr) {
      ...
      if (bookData.isActive) {
        const {
          ...
          marginToAdd,
          newBookAnimations,
        } = await thisBot.HandleBookDataInStack({
          dimension,
          duration,
          easing,
          bookData,
          ...
          desiredPositionZ:
            nextPositionZ +
            (sectionData.piece.masks.isOnTheGround
              ? BibleVizUtils.Data.tags.StackSpacing.SectionShadowPadding
              : 0),
          speedMultiplier,
          isInstantaneous,
        });
        ...
        const tempSectionShadowScales = {
          x: absBookDesiredPosition.x + halfInitialBookScales.x,
          y: absBookDesiredPosition.y + halfInitialBookScales.y,
        };
        if (tempSectionShadowScales.x > sectionShadowDesiredScales.x)
          sectionShadowDesiredScales.x = tempSectionShadowScales.x;
        if (tempSectionShadowScales.y > sectionShadowDesiredScales.y)
          sectionShadowDesiredScales.y = tempSectionShadowScales.y;
    ...
    if (
      bookDataArr.some((bookData: any) => {
        return bookData.isActive;
      }) &&
      !sectionData.isInExplodedView
    )
      nextPositionZ +=
        bookDataArr.find((bookData: any) => {
          return bookData.isActive;
        }).piece.tags.desiredScaleZ +
        BibleVizUtils.Data.tags.StackSpacing.BetweenBooks;
  }
  sectionShadowDesiredScales.x =
    sectionShadowDesiredScales.x * 2 +
    BibleVizUtils.Data.tags.StackSpacing.SectionShadowPadding;
  sectionShadowDesiredScales.y =
    sectionShadowDesiredScales.y * 2 +
    BibleVizUtils.Data.tags.StackSpacing.SectionShadowPadding;
  if (activeBooksInsideSection.length === 0) {
    sectionShadowDesiredScales.z =
      BibleVizUtils.Data.tags.StackPieceMeasurements.EmptySectionShadowScaleZ;
    ...
    if (sectionData.isInExplodedView) {
      ...
      const rawActiveBooksInsideSecion = sectionData.childrenData.filter(
        (bookDataArr: any) => {
          return bookDataArr.some((bookData: any) => {
            return bookData.isActive;
          });
        }
      );
      const booksTotalScaleZ = rawActiveBooksInsideSecion.reduce(
        (total: any, currentRawBookData: any) => {
          return (
            total +
            currentRawBookData.find((bookData: any) => {
              return bookData.isActive;
            }).piece.tags.desiredScaleZ
          );
        },
  ...
  if (sectionData.shadow) {
    ...
    const infoLabelTransformer =
      BibleVizUtils.Functions.GetCurrentInfoLabelTransformer(
        sectionData.shadow
      );
    ...
    if (infoLabelTransformer && !sectionData.isInExplodedView) {
      newSectionAnimations.push(
        infoLabelTransformer
          .Hide({ speedMultiplier, isInstantaneous })
          .then(() => {
    ...
    if (isInstantaneous) {
      ...
      setTagMask(
        sectionData.shadow,
        "scaleX",
        sectionData.isInExplodedView && activeBooksInsideSection.length > 0
          ? sectionShadowDesiredScales.x
      ...
      if (
        ...
        const label = BibleVizUtils.Functions.CapitalizeFirstLetter(
  ...
  if (activeBooksInsideSection.length === 0)
    nextPositionZ =
      sectionShadowDesiredPositionZ + sectionShadowDesiredScales.z;
  ...
  if (sectionData.isActive) {
    const isSectionBookDataInstance =
      ...
      sectionData.constructor.name === "StackSectionBookData"; // instanceof not working the first time for some reason so checking by name;
    if (isSectionBookDataInstance) {
      ...
      const sectionCurrentScales = BibleVizUtils.Functions.GetBotScales(
        sectionData.piece
      );
      ...
      const setFormOpacity =
        sectionData.piece.tags.formOpacity !=
        sectionData.piece.tags.unhoveredOpacity;
      if (isInstantaneous) {
        if (setScaleX)
          setTagMask(sectionData.piece, "scaleX", sectionDesiredScales.x);
        if (setScaleY)
          setTagMask(sectionData.piece, "scaleY", sectionDesiredScales.y);
        if (setScaleZ)
          setTagMask(sectionData.piece, "scaleZ", sectionDesiredScales.z);
        if (setFormOpacity)
          setTagMask(
            sectionData.piece,
            "formOpacity",
    ...
    if (isInstantaneous)
      setTagMask(sectionData.piece, dimension + "Z", nextPositionZ);
    else
      newSectionAnimations.push(
