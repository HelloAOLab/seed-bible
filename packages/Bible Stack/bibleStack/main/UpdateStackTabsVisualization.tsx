...
const activeTab = thisBot.vars.tabsContext.tabs.find((tab: any) => {
...
if (activeTab) {
  ...
  thisBot.vars.stackChaptersData.forEach((chapterData: any) => {
  ...
  if (chapterToFocus) {
    const bookData = thisBot.vars.stackBooksData.find((currBookData: any) => {
    ...
    const sectionData = thisBot.vars.stackSectionsData.find((data: any) => {
      return data.id === chapterToFocus.parentDataIds.stackSectionId;
    });
    const testamentData = thisBot.vars.stackTestamentsData.find((data: any) => {
    ...
    const speedMultiplier =
      shouldResetStack || speedMultiplierConditions.filter(Boolean).length > 1
        ? 2
        : 1;
    const getBookToDeselect = (currBookData: any) => {
      return (
