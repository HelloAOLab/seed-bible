...
const createGameButton = getBot(
  byTag("isCreateGameButton", true),
  byTag("isInitialized", true)
);
...
const onClickContinue = (verse: any, refer) => {
...
const SelectedBook = () => {
  const verseList = useMemo(
    () =>
      trophy.tags.bibleVerse[thisBot.tags.bookName].map((verseFull: any) =>
        trophy.splitVerserAndReference({
          bookName: thisBot.tags.bookName,
          verse: verseFull,
        })
      ),
      ...
      >
        <div className="book-style">
          <div className="book-style-container">
            <p className="select-book-text">
              You've selected <b>{thisBot.tags.bookName}</b>.
            </p>
            <p>
              Get ready to <b>swap seats!</b>
            </p>
            ...
            <div className="guess-select-controls">
              <Button
                onClick={() =>
                  setSelected(
                    (p) => (p - 1 + verseList.length) % verseList.length
                  )
                }
              >
                «
              ...
              <Button
                onClick={() => setSelected((p) => (p + 1) % verseList.length)}
              >
                »
              </Button>
            </div>
            <div className="book-style-buttons">
              ...
              <p
                style={{
                  background: VERESE_BG_HEX_CODE[selected].hex,
                  border: `1px solid ${VERESE_BG_HEX_CODE[selected].border}`,
                  padding: "12px",
                  borderRadius: "8px",
                  fontSize: "12px",
                  fontWeight: "800",
