...
const testamentsScales = bibleData.childrenData.map((testamentData: any) => {
  return BibleVizUtils.Functions.GetBotScales(testamentData.piece);
});
...
const testamentsPositionZ = bibleData.childrenData.map(
  (testamentData: any, index) => {
    return (
      lowerCoverPosition.z +
      lowerCoverScales.z +
      BibleVizUtils.Data.tags.StackSpacing.BetweenArrangements * (index + 1) +
...
const upperCoverPositionZ =
  ...
  BibleVizUtils.Data.tags.StackSpacing.BetweenArrangements;
...
const crossPositionZ =
  upperCoverPositionZ +
  upperCoverScales.z +
  BibleVizUtils.Data.tags.StackSpacing.CoverToCross;
const animations = [];

bibleData.childrenData.forEach((testamentData: any, index) => {
...
await Promise.all(animations)
  .then(() => {
    setTagMask(
      bibleData.childrenData.map((testamentData: any) => {
