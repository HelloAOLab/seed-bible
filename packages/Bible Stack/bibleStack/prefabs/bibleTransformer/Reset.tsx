...
return thisBot.CloseBible({ duration, easing, bibleData }).then(() => {
  shout("OnStackBibleCloseComplete", { bibleData });
  return thisBot.OpenBible({ duration, easing, bibleData }).then(() => {
    shout("OnStackBibleResetComplete", { bibleData });
  });
});
