...
import type { AwaiterPort } from "bibleStack.application.ports.experience";
import type {
  ...
  async closeBible({
    ...
    const selectedBooks: (Piece<"StackSectionBook"> | Piece<"StackBook">)[] = [
      ...sectionsData.flatMap((data) =>
        data.type === "StackSectionBook" && data.isSelected && data.piece
          ? [data.piece]
          : []
      ),
      ...booksData.flatMap((data) =>
        data.isSelected && data.piece ? [data.piece] : []
      ),
    ];
