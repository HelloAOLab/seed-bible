...
- Offer to switch to a downloaded translation when a chapter fails to load, if another translation in the same language is saved on the device. When more than one matches, they are listed by name and you pick one from the same searchable menu Settings uses for language.
