...
typeof navigator != "undefined"
  ...
  : typeof os != "undefined" && os.platform && os.platform();
//#endregion
...
//#endregion
//#region node_modules/.pnpm/prosemirror-view@1.42.0/node_modules/prosemirror-view/dist/index.js
  ...
  for (;;) {
    ...
    if (off == (dir < 0 ? 0 : nodeSize(node))) {
      ...
      if (child.nodeType == 1 && child.contentEditable == "false")
        if (
          (_a = child.pmViewDesc) === null || _a === void 0
            ? void 0
            : _a.ignoreForSelection
        )
          off += dir;
        else return false;
      else {
...
function posAtCoords(view, coords) {
  ...
  let elt = (view.root.elementFromPoint ? view.root : doc).elementFromPoint(
    coords.left,
    coords.top
  );
  let pos;
...
var NodeViewDesc = class NodeViewDesc extends ViewDesc {
  ...
  static create(parent, node, outerDeco, innerDeco, view, pos) {
    ...
    if (node.isText) {
      ...
      let spec = DOMSerializer.renderSpec(
        document,
    ...
    iterDeco(
      ...
      (widget, i, insideNode) => {
        if (widget.spec.marks)
          ...
          updater.syncToMarks(
            i == this.node.childCount ? Mark$1.none : this.node.child(i).marks,
            inline,
            view,
            i
          );
...
function addToSibling(wrap, lastWrap, node, sibling, depth) {
  if (
    ...
    if (
      sibling
        .contentMatchAt(sibling.childCount)
        .matchType(depth == wrap.length - 1 ? node.type : wrap[depth + 1])
    )
      return sibling.copy(
...
function initInput(view) {
  for (let event in handlers) {
    ...
    view.dom.addEventListener(
      event,
      (view.input.eventHandlers[event] = (event) => {
        if (
...
var TripleClickDrag = class extends MouseDown {
  ...
  move(event) {
    if (
      ...
      !this.view.state.doc.eq(this.startDoc)
    ) {
      this.done();
      return;
    }
    ...
    let [anchor, head] =
      target.from < start.from
        ? [start.to, target.from]
  ...
  if (!view.composing) {
    ...
    if (
      ...
      (state.storedMarks ||
        ...
        (chrome && windows$1 && selectionBeforeUneditable(view)))
    ) {
...
var brokenClipboardAPI =
  (ie$1 && ie_version < 15) || (ios && webkit_version < 604);
...
editHandlers.paste = (view, _event) => {
  ...
  if (
    data &&
    doPaste(view, getText$1(data), data.getData("text/html"), plain, event)
  )
    event.preventDefault();
...
handlers.dragstart = (view, _event) => {
  ...
  let { dom, text, slice } = serializeForClipboard(
    view,
    (node || view.state.selection).content()
  );
      ...
      default:
        return new DecorationGroup(
          members.every((m) => m instanceof DecorationSet)
            ? members
            : members.reduce(
                (r, m) => r.concat(m instanceof DecorationSet ? m : m.members),
                []
              )
        );
    }
    ...
    if (
      ...
      if (
        (movedPos > 0 &&
          ...
          this.state.doc.nodeAt(movedPos)) == sel.node
      )
        found = movedPos;
    }
...
var deleteCurrentNode =
  ...
  ({ tr, dispatch }) => {
    ...
    for (let depth = $pos.depth; depth > 0; depth -= 1)
      if ($pos.node(depth).type === currentNode.type) {
        if (dispatch) {
...
var expandSelectionForSide = ($pos, schema, side) => {
  ...
  if (
    (side === "left" && $pos.pos > $pos.start()) ||
    (side === "right" && $pos.pos < $pos.end())
  )
    return $pos.pos;
...
function findMarkInSet(marks, type, attributes = {}) {
  return marks.find((item) => {
    return (
      ...
      objectIncludes(
        Object.fromEntries(
          Object.keys(attributes).map((k) => [k, item.attrs[k]])
        ),
        attributes
      )
    );
    ...
    if (options.errorOnInvalidContent) {
      ...
      if (options.errorOnInvalidContent && hasInvalidContent)
        throw new Error("[tiptap error]: Invalid HTML content", {
          cause: /* @__PURE__ */ new Error(
            `Invalid element found: ${invalidContent}`
          ),
        });
    }
      ...
      try {
        content = createNodeFromContent(value, editor.schema, {
          ...
          errorOnInvalidContent:
            (_a = options.errorOnInvalidContent) != null
              ? _a
              : editor.options.enableContentCheck,
        });
      } catch (e) {
...
var lift =
  (typeOrName, attributes = {}) =>
  ({ state, dispatch }) => {
    if (!isNodeActive(state, getNodeType(typeOrName, state.schema), attributes))
      return false;
...
var liftListItem =
  (typeOrName) =>
  ({ state, dispatch }) => {
    return liftListItem$1(getNodeType(typeOrName, state.schema))(
      state,
      dispatch
    );
  };
...
var setContent =
  (
    content,
    { errorOnInvalidContent, emitUpdate = true, parseOptions = {} } = {}
  ) =>
...
function getAttributesFromExtensions(extensions) {
  ...
  nodeAndMarkExtensions.forEach((extension) => {
    ...
    Object.entries(attributes).forEach(([name, attribute]) => {
      ...
      if (
        typeof (mergedAttr == null ? void 0 : mergedAttr.default) === "function"
      )
...
function buildAttributeSpec(extensionAttribute) {
  ...
  if (
    !((_a =
      extensionAttribute == null ? void 0 : extensionAttribute.attribute) ==
    null
      ? void 0
      : _a.isRequired) &&
    "default" in
...
function getTextBetween(startNode, range, options) {
  ...
  startNode.nodesBetween(from, to, (node, pos, parent, index) => {
    ...
    const textSerializer =
      textSerializers == null ? void 0 : textSerializers[node.type.name];
    if (textSerializer) {
      if (parent)
        text += textSerializer({
          node,
...
function getDebugJSON(node, startOffset = 0) {
  ...
  if (node.content.childCount) {
    ...
    node.forEach((child, offset) => {
      var _a;
      (_a = output.content) == null ||
        _a.push(getDebugJSON(child, startOffset + offset + increment));
    });
  }
...
function isNodeEmpty(
  node,
  { checkChildren = true, ignoreWhitespace = false } = {}
) {
  var _a;
  if (ignoreWhitespace) {
...
function rewriteUnknownContentInner({
  ...
  if (json.type && !validNodes.has(json.type)) {
    ...
    if (
      ...
      (options == null ? void 0 : options.fallbackToParagraph) !== false
    ) {
      json.type = "paragraph";
      return {
        json,
      ...
      if (empty) {
        ...
        ranges.forEach((range) => {
          ...
          state.doc.nodesBetween(from, to, (node, pos) => {
            ...
            if (node.marks.find((mark) => mark.type === type))
              node.marks.forEach((mark) => {
                if (type === mark.type)
                  tr.addMark(
                    trimmedFrom,
...
var sinkListItem =
  (typeOrName) =>
  ({ state, dispatch }) => {
    return sinkListItem$1(getNodeType(typeOrName, state.schema))(
      state,
      dispatch
    );
  };
...
var joinListBackwards = (tr, listType) => {
  ...
  if (
    !(
      list.node.type === (nodeBefore == null ? void 0 : nodeBefore.type) &&
...
var joinListForwards = (tr, listType) => {
  ...
  if (
    !(
      list.node.type === (nodeAfter == null ? void 0 : nodeAfter.type) &&
  ...
  if (
    !areListTypesCompatible(
      list.node.attrs.type,
      nodeAfter == null ? void 0 : nodeAfter.attrs.type
    )
  )
    return true;
  tr.join(after);
...
var toggleList =
  ...
  ({ editor, tr, state, dispatch, chain, commands, can }) => {
    ...
    const soleTopLevelNode =
      topLevelNodes.length === 1 ? topLevelNodes[0] : null;
...
var wrapIn =
  (typeOrName, attributes = {}) =>
  ({ state, dispatch }) => {
    return wrapIn$1(getNodeType(typeOrName, state.schema), attributes)(
      state,
      dispatch
    );
  };
var wrapInList =
  (typeOrName, attributes = {}) =>
  ({ state, dispatch }) => {
    return wrapInList$1(getNodeType(typeOrName, state.schema), attributes)(
      state,
      dispatch
    );
  };
...
function canInsertNode(state, nodeType) {
  ...
  while (depth >= 0) {
    ...
    if ($from.node(depth).contentMatchAt(index).matchType(nodeType))
      return true;
    depth -= 1;
  }
  return false;
...
function getStyleProperty(element, propertyName) {
  ...
  for (let i = decls.length - 1; i >= 0; i -= 1) {
    ...
    if (decl.slice(0, colonIndex).trim().toLowerCase() === target)
...
function parseAttributes(attrString) {
  ...
  Array.from(
    ...
    const quotedValue =
      quotedStrings[
        parseInt(
          ((_a = quotedRef.match(/__QUOTED_(\d+)__/)) == null
            ? void 0
            : _a[1]) || "0",
          10
        )
  ...
  return {
    ...
    markdownTokenizer: {
      ...
      tokenize(src, _tokens, _lexer) {
        ...
        if (requiredAttributes.find((required) => !(required in attributes)))
          return;
        return {
          type: nodeName,
          raw: match[0],
...
function createInlineMarkdownSpec(options) {
  ...
  const filterAttributes = (attrs) => {
    ...
    allowedAttributes.forEach((attr) => {
      ...
      const skipIfDefault =
        typeof attr === "string" ? void 0 : attr.skipIfDefault;
...
`;
    ...
    while (i < lines.length) {
      ...
      if (nextLine.trim() === "") {
        ...
        if (
          (((_b =
            (_a = lines[i + 1 + nextNonEmptyIndex].match(/^(\s*)/)) == null
              ? void 0
              : _a[1]) == null
            ? void 0
...
`;
      ...
      if (
        (((_d = (_c = nextLine.match(/^(\s*)/)) == null ? void 0 : _c[1]) ==
        null
          ? void 0
...
function renderNestedMarkdownContent(node, h2, prefixOrGenerator, ctx) {
  ...
  const prefix =
    typeof prefixOrGenerator === "function"
  ...
  if (children && children.length > 0)
    children.forEach((child, index) => {
      ...
      const childContent =
        (_b =
          (_a = h2.renderChild) == null
            ? void 0
            : _a.call(h2, child, index + 1)) != null
          ? _b
...
var Mark = class _Mark extends Extendable {
  ...
  static handleExit({ editor, mark }) {
    ...
    if (currentPos.pos === currentPos.end()) {
      ...
      if (
        !!!currentMarks.find(
          (m) => (m == null ? void 0 : m.type.name) === mark.name
        )
      )
        return false;
      ...
      props: {
        handleDOMEvents: {
          ...
          paste: (_view, event) => {
            var _a;
            const html =
              (_a = event.clipboardData) == null
                ? void 0
                : _a.getData("text/html");
            pasteEvent = event;
            isPastedFromProseMirror = !!(html == null
              ? void 0
...
var ExtensionManager = class {
  ...
  get plugins() {
    const { editor } = this;
    return sortExtensions([...this.extensions].reverse()).flatMap(
      (extension) => {
        const context = {
          name: extension.name,
          options: extension.options,
  ...
  dispatchTransaction(baseDispatch) {
    const { editor } = this;
    return sortExtensions([...this.extensions].reverse()).reduceRight(
      (next, extension) => {
        const context = {
          name: extension.name,
          options: extension.options,
...
var Delete = Extension.create({
  ...
  onUpdate({ transaction, appendedTransactions }) {
    ...
    const callback = () => {
      ...
      if (
        (_d =
          (_c2 =
            (_b2 =
              (_a2 = this.editor.options.coreExtensionOptions) == null
                ? void 0
                : _a2.delete) == null
              ? void 0
      ...
      getChangedRanges(nextTransaction).forEach((change) => {
        if (
          nextTransaction.mapping.mapResult(change.oldRange.from)
            .deletedAfter &&
          ...
          nextTransaction.before.nodesBetween(
            ...
            (node, from) => {
              ...
              const isFullyWithinRange =
                change.oldRange.from <= from && to <= change.oldRange.to;
    ...
    if (
      (_c =
        (_b =
          (_a = this.editor.options.coreExtensionOptions) == null
            ? void 0
            : _a.delete) == null
          ? void 0
          : _b.async) != null
  ...
  querySelectorAll(selector, attributes = {}, firstItemOnly = false) {
    ...
    this.children.forEach((childPos) => {
      ...
      if (childPos.node.type.name === selector) {
        if (
          attrKeys.every((key) => attributes[key] === childPos.node.attrs[key])
        )
          nodes.push(childPos);
      }
      if (firstItemOnly && nodes.length > 0) return;
...
var Editor = class extends EventEmitter {
  ...
  createExtensionManager() {
    ...
    const allExtensions = [
      ...(this.options.enableCoreExtensions
        ? [
            ...
            Tabindex.configure({
              value:
                (_d =
                  (_c = this.options.coreExtensionOptions) == null
                    ? void 0
                    : _c.tabindex) == null
                  ? void 0
                  : _d.value,
      ...
      return ["extension", "node", "mark"].includes(
        extension == null ? void 0 : extension.type
      );
    });
  ...
  createView(element) {
    ...
    const baseDispatch =
      editorProps.dispatchTransaction || this.dispatchTransaction.bind(this);
  ...
  dispatchTransaction(transaction) {
    ...
    const focus2 =
      mostRecentFocusTr == null ? void 0 : mostRecentFocusTr.getMeta("focus");
    const blur2 =
      mostRecentFocusTr == null ? void 0 : mostRecentFocusTr.getMeta("blur");
    if (focus2)
      this.emit("focus", {
        editor: this,
        event: focus2.event,
  ...
  get isDestroyed() {
    var _a, _b;
    return (_b = (_a = this.editorView) == null ? void 0 : _a.isDestroyed) !=
      null
      ? _b
      : true;
  }
  $node(selector, attributes) {
    ...
    return (
      ((_a = this.$doc) == null
        ? void 0
        : _a.querySelector(selector, attributes)) || null
    );
  }
  $nodes(selector, attributes) {
    ...
    return (
      ((_a = this.$doc) == null
        ? void 0
        : _a.querySelectorAll(selector, attributes)) || null
    );
  }
  $pos(pos) {
    const $pos = this.state.doc.resolve(pos);
    const node =
      pos > 0 && $pos.nodeAfter && !$pos.nodeAfter.isText
        ? $pos.nodeAfter
        : null;
      ...
      if (captureGroup) {
        ...
        if (
          getMarksBetween(range.from, range.to, state.doc)
            .filter((item) => {
              return item.mark.type.excluded.find(
                (type) => type === config.type && type !== item.mark.type
              );
            })
            .filter((item) => item.to > textStart).length
        )
          return null;
...
function nodeInputRule(config) {
  return new InputRule({
    ...
    handler: ({ state, range, match }) => {
      const attributes =
        callOrReturn(config.getAttributes, void 0, match) || {};
      const { tr } = state;
...
var ResizableNodeView = class {
  ...
  constructor(options) {
    ...
    if (
      (_c = options == null ? void 0 : options.options) == null
        ? void 0
        : _c.directions
    )
  ...
  calculateNewDimensions(direction, deltaX, deltaY) {
    ...
    if (this.preserveAspectRatio || this.isShiftKeyPressed)
...
var NodeView = class {
  ...
  onDragStart(event) {
    ...
    const dragHandle =
      target.nodeType === 3
        ? (_a = target.parentElement) == null
          ? void 0
          : _a.closest("[data-drag-handle]")
        : target.closest("[data-drag-handle]");
    if (
      !this.dom ||
      ((_b = this.contentDOM) == null ? void 0 : _b.contains(target)) ||
      !dragHandle
    )
      return;
    let x = 0;
    ...
    if (this.dom !== dragHandle) {
      ...
      const offsetX =
        (_d = event.offsetX) != null
          ? _d
          : (_c = event.nativeEvent) == null
            ? void 0
            : _c.offsetX;
      const offsetY =
        (_f = event.offsetY) != null
          ? _f
          : (_e = event.nativeEvent) == null
            ? void 0
            : _e.offsetY;
  ...
  stopEvent(event) {
    ...
    if (
      !(
        ...
        !((_a = this.contentDOM) == null ? void 0 : _a.contains(target))
      )
    )
      return false;
    const isDragEvent = event.type.startsWith("drag");
    const isDragOverEnterEvent =
...
//#endregion
export {
  getMarkAttributes as $,
  parseAttributes as $t,
  decodeHtmlEntities as A,
  isMacOS as At,
  findParentNode as B,
  isString as Bt,
  createBlockMarkdownSpec as C,
  isAtEndOfNode as Ct,
  createMappablePosition as D,
  isFirefox as Dt,
  createInlineMarkdownSpec as E,
  isExtensionRulesEnabled as Et,
  escapeForRegEx as F,
  isNodeViewSelected as Ft,
  generateJSON as G,
  markdown_exports as Gt,
  flattenExtensions as H,
  isiOS as Ht,
  extensions_exports as I,
  isNumber as It,
  getAttributesFromExtensions as J,
  mergeDeep as Jt,
  generateText as K,
  marksEqual as Kt,
  findChildren as L,
  ...
  deleteProps as M,
  isNodeActive as Mt,
  elementFromString as N,
  isNodeEmpty as Nt,
  createNodeFromContent as O,
  isFunction as Ot,
  encodeHtmlEntities as P,
  isNodeSelection as Pt,
  getHTMLFromFragment as Q,
  objectIncludes as Qt,
  findChildrenInRange as R,
  isRegExp as Rt,
  createAtomBlockMarkdownSpec as S,
  isAndroid as St,
  createDocument as T,
  isEmptyObject as Tt,
  fromString as U,
  markInputRule as Ut,
  findParentNodeClosestToPos as V,
  isTextSelection as Vt,
  generateHTML as W,
  markPasteRule as Wt,
  getDebugJSON as X,
  nodeInputRule as Xt,
  getChangedRanges as Y,
  minMax as Yt,
  getExtensionField as Z,
  nodePasteRule as Zt,
  attrsEqual as _,
  keydownHandler as _n,
  getUpdatedPosition as _t,
  Fragment6 as a,
  resolveExtensions as an,
  getNodeType as at,
  combineTransactionSteps as b,
  inputRulesPlugin as bt,
  Mark as c,
  selectionToInsertionEnd as cn,
  getSchemaByResolvedExtensions as ct,
  NodePos as d,
  splitExtensions as dn,
  getSplittedAttributes as dt,
  parseIndentedBlocks as en,
  getMarkRange as et,
  NodeView as f,
  textInputRule as fn,
  getStyleProperty as ft,
  Tracker as g,
  wrappingInputRule as gn,
  getTextSerializersFromSchema as gt,
  ResizableNodeview as h,
  updateMarkViewAttributes as hn,
  getTextContentFromNodes as ht,
  Extension as i,
  renderNestedMarkdownContent as in,
  getNodeAttributes as it,
  defaultBlockAt as j,
  isMarkActive as jt,
  createStyleTag as k,
  isList as kt,
  MarkView as l,
  serializeAttributes as ln,
  getSchemaTypeByName as lt,
  ResizableNodeView as m,
  textblockTypeInputRule as mn,
  getTextBetween as mt,
  Editor as n,
  posToDOMRect as nn,
  getMarksBetween as nt,
  InputRule as o,
  resolveFocusPosition as on,
  getRenderedAttributes as ot,
  PasteRule as p,
  textPasteRule as pn,
  getText as pt,
  getAttributes as q,
  mergeAttributes as qt,
  Extendable as r,
  removeDuplicates as rn,
  getNodeAtPosition as rt,
  MappablePosition as s,
  rewriteUnknownContent as sn,
  getSchema as st,
  CommandManager as t,
  pasteRulesPlugin as tn,
  getMarkType as tt,
  Node3 as u,
  sortExtensions as un,
  getSchemaTypeNameByName as ut,
  callOrReturn as v,
  Decoration as vn,
  h as vt,
  createChainableState as w,
  isAtStartOfNode as wt,
  commands_exports as x,
  isActive as xt,
  canInsertNode as y,
  DecorationSet as yn,
  injectExtensionAttributesToParseRule as yt,
  findDuplicates as z,
  isSafari as zt,
};

//# sourceMappingURL=dist-DQVaiYlO.js.map
