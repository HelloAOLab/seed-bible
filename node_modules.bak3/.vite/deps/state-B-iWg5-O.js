...
function checkAttrs(attrs, values, type, name) {
  for (let attr in values)
    if (!(attr in attrs))
      throw new RangeError(
        `Unsupported attribute ${attr} for ${type} of type ${name}`
      );
    ...
    if (parent != content && this.find) {
      for (let i = 0; i < this.find.length; i++)
        if (
          ...
          if (
            content.compareDocumentPosition(this.find[i].node) &
            (before ? 2 : 4)
          )
            this.find[i].pos = this.currentPos;
        }
...
function liftTarget(range) {
  let content = range.parent.content.cutByIndex(
    range.startIndex,
    range.endIndex
  );
  for (
...
function findWrappingInside(range, type) {
  ...
  let innerMatch = (inside.length ? inside[inside.length - 1] : type)
    .contentMatch;
    ...
    if (
      ...
      if (type.schema.linebreakReplacement) {
        let pre = type.whitespace == "pre",
          supportLinebreak = !!type.contentMatch.matchType(
...
//# sourceMappingURL=state-B-iWg5-O.js.map
