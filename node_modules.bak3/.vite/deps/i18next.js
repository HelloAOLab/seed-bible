//#region node_modules/.pnpm/i18next@26.2.0_typescript@6.0.3/node_modules/i18next/dist/esm/i18next.js
...
var looksLikeObjectPath = (key, nsSeparator, keySeparator) => {
  ...
  const possibleChars = chars.filter(
    (c) => !nsSeparator.includes(c) && !keySeparator.includes(c)
  );
...
var deepFind = (obj, path, keySeparator = ".") => {
  ...
  for (let i = 0; i < tokens.length; ) {
    ...
    for (let j = i; j < tokens.length; ++j) {
      ...
      if (next !== void 0) {
        if (
          ["string", "number", "boolean"].includes(typeof next) &&
          j < tokens.length - 1
        )
          continue;
        i += j - i + 1;
...
var EventEmitter = class {
  ...
  emit(event, ...args) {
    if (this.observers[event])
      Array.from(this.observers[event].entries()).forEach(
        ([observer, numTimesAdded]) => {
          for (let i = 0; i < numTimesAdded; i++) observer(...args);
        }
      );
    if (this.observers["*"])
      Array.from(this.observers["*"].entries()).forEach(
        ([observer, numTimesAdded]) => {
...
var postProcessor = {
  ...
  handle(processors, value, key, options, translator) {
    processors.forEach((processor) => {
      value =
        this.processors[processor]?.process(value, key, options, translator) ??
        value;
    });
    return value;
  },
...
function keysFromSelector(selector, opts) {
  ...
  if (path.length > 1 && nsSeparator) {
    ...
    if (nsList) {
      if (
        (strict ? nsList : nsList.length > 1 ? nsList.slice(1) : []).includes(
          path[0]
        )
      )
...
var Translator = class Translator extends EventEmitter {
  ...
  extractFromKey(key, opt) {
    let nsSeparator =
      opt.nsSeparator !== void 0 ? opt.nsSeparator : this.options.nsSeparator;
    if (nsSeparator === void 0) nsSeparator = ":";
    const keySeparator =
      opt.keySeparator !== void 0
        ? opt.keySeparator
        : this.options.keySeparator;
  ...
  translate(keys, o, lastKey) {
    ...
    const joinArrays =
      opt.joinArrays !== void 0 ? opt.joinArrays : this.options.joinArrays;
    ...
    const needsZeroSuffixLookup =
      needsPluralHandling && !opt.ordinal && opt.count === 0;
    const defaultValue =
...
var LanguageUtil = class {
  ...
  isSupportedCode(code) {
    ...
    return (
      ...
      this.supportedLngs.includes(code)
    );
  }
  ...
  toResolveHierarchy(code, fallbackCode) {
    const fallbackCodes = this.getFallbackCodes(
      (fallbackCode === false ? [] : fallbackCode) ||
        this.options.fallbackLng ||
        [],
      code
    );
...
var Interpolator = class {
  ...
  init(options = {}) {
    ...
    this.unescapePrefix = unescapeSuffix
      ? ""
      : unescapePrefix
        ? regexEscape(unescapePrefix)
        : "-";
  ...
  nest(str, fc, options = {}) {
    ...
    const handleHasOptions = (key, inheritedOptions) => {
      ...
      if (
        ...
        clonedOptions.defaultValue.includes(this.prefix)
      )
...
var Formatter = class {
  ...
  init(services, options = { interpolation: {} }) {
    ...
    const cf = options.cacheInBuiltFormats
      ? createCachedFormatter
      : createNonCachedFormatter;
    this.formats = {
      number: cf((lng, opt) => {
