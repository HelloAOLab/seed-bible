...
var peg$SyntaxError = class extends SyntaxError {
  ...
  format(sources) {
    ...
    if (this.location) {
      ...
      if (src) {
        ...
        const hatLen =
          (s.line === e.line ? e.column : line.length + 1) - s.column || 1;
        str +=
          "\n --> " +
          loc +
          "\n" +
  ...
  static buildMessage(expected, found) {
    ...
    const nonPrintable = Object.prototype.hasOwnProperty.call(
      ...
      ? /* @__PURE__ */ new RegExp("[\\p{C}\\p{Mn}\\p{Mc}]", "gu")
      : null;
...
function peg$parse(input, options) {
  ...
  function peg$f0(val_1, sep_val, seq_post) {
    if (sep_val && sep_val.type && sep_val.type === "c_explicit")
      seq_post.explicit_context = "c";
    return [seq_post];
  }
  ...
  function peg$f3(sep_val_1, val_1, val_2) {
    ...
    if (sep_val_1 && sep_val_1.type && sep_val_1.type === "c_explicit")
      val_1.explicit_context = "c";
    val_2.unshift([val_1]);
    const r = range();
    return {
  ...
  function peg$parseinteger_value() {
    let res;
    if (
      (res = /^[0-9]{1,3}(?![0-9]|,000)/.exec(input.substring(peg$currPos)))
    ) {
      peg$savedPos = peg$currPos;
      peg$currPos += res[0].length;
      return [res[0]];
...
var bcv_matcher = class {
  ...
  match_passages(s) {
    ...
    for (const match of s.matchAll(this.parent.regexps.escaped_passage)) {
      ...
      const start_index_adjust = part.startsWith("")
        ? 0
        : part.split("")[0].length;
      const passage = {
        ...
        type: "base",
        start_index:
          this.parent.passage.books[book_id_number].start_index -
          start_index_adjust,
        match: part,
      };
      ...
      const regexp_index_adjust = this.adjust_regexp_end(
        accum,
  ...
  create_book_range(s, passage, book_id) {
    ...
    const limit = parseInt(
      this.parent.passage.books[book_id].parsed[0].substring(0, 1),
      10
    );
    for (let i = 1; i < limit; i++) {
      ...
      const match_regexp = new RegExp(
        String.raw`${this.parent.regexps.pre_number_book.source}(${cases[i - 1].source}\s*${range_regexp.source}\s*)\x1f${book_id}\x1f`,
        "iu"
      );
...
var bcv_options = class {
  constructor(parent) {
    ...
    this.#grammar = {
      ...
      and: /^&/,
      c_explicit:
        /^[\s*]*(?:chapters?|cha?pts?\.?|chaps?\.?|ch[aps]?\.?)[\s*]*/i,
  ...
  set testaments(filter) {
    ...
    if (out.length > 0 && out !== this.#testaments) {
      ...
      if (new_apocrypha !== old_apocrypha) this.set_apocrypha(new_apocrypha);
        ...
        this.parent.regexps_manager.filter_books(
          this.#testaments,
          this.case_sensitive
        );
    }
  ...
  set_apocrypha(include_apocrypha) {
    ...
    for (const translation of Object.keys(this.parent.translations.systems)) {
      ...
      this.parent.translations.systems[translation].chapters["Ps"] ??= [
        ...this.parent.translations.systems.current.chapters["Ps"],
      ];
      if (include_apocrypha === true)
        this.parent.translations.systems[translation].chapters["Ps"][150] =
          this.parent.translations.systems[translation].chapters[
            "Ps151"
          ]?.[0] ??
          this.parent.translations.systems.current.chapters["Ps151"][0];
      else if (
        this.parent.translations.systems[translation].chapters?.["Ps"]
          .length === 151
      )
  ...
  set versification_system(system) {
    ...
    if (!this.parent.translations.systems[system]) {
      if (this.warn_level === "warn")
        console.warn(
          `Unknown versification system ("${system}"). Using default instead.`
        );
      system = "default";
    }
    if (!system || system === this.parent.translations.current_system) return;
    if (this.parent.translations.current_system !== "default")
      this.parent.translations.systems.current = structuredClone(
        this.parent.translations.systems.default
      );
    this.parent.translations.current_system = system;
    if (system === "default") return;
    if (this.parent.translations.systems[system].order)
      this.parent.translations.systems.current.order = {
        ...this.parent.translations.systems[system].order,
      };
    if (this.parent.translations.systems[system].chapters)
      this.parent.translations.systems.current.chapters = {
  ...
  set case_sensitive(arg) {
    if (
      arg === this.#case_sensitive ||
      (arg !== "none" &&
        arg !== "books" &&
        arg !== "translations" &&
        arg !== "books,translations")
    )
      return;
  ...
  set punctuation_strategy(arg) {
    if (arg !== "us" && arg !== "eu") {
      if (this.warn_level === "warn")
        console.warn(
          `punctuation_strategy should be set to "us" or "eu", not: ${arg}`
        );
      return;
    }
...
var bcv_passage = class {
  ...
  b(passage, accum, context) {
    ...
    if (passage.start_context.translations)
      context.translations = structuredClone(
  ...
  b_range_pre(passage, accum, context) {
    ...
    if (passage.start_context.translations)
      passage.passages[0].translations = structuredClone(
        passage.start_context.translations
      );
    accum.push(passage);
    return [accum, context];
  ...
  bc(passage, accum, context) {
    ...
    for (const b of this.books[this.pluck("b", passage.value).value].parsed) {
      ...
      [obj.start.c, obj.start.v] = this.fix_start_zeroes(
        obj.valid,
        obj.start.c,
        obj.start.v
      );
    ...
    this.normalize_passage_and_alternates(
      passage,
      alternates,
      adjust_end_index_by
    );
    this.set_context_from_object(
      context,
      ["b", "c", "v"],
  ...
  bc_title(passage, accum, context) {
    ...
    if (
      bc.passages[0].start.b.substring(0, 2) !== "Ps" &&
      bc.passages[0].alternates
    ) {
  ...
  c(passage, accum, context) {
    passage.start_context = structuredClone(context);
    const chapter_integer =
      passage.type === "integer"
        ? passage
        : this.pluck("integer", passage.value);
  ...
  next_v(passage, accum, context) {
    passage.start_context = structuredClone(context);
    const prev_integer = this.pluck_last_recursively(
      "integer",
      passage.value
    ) ?? { value: 1 };
    passage.value.push({
  ...
  sequence(passage, accum, context) {
    ...
    for (const obj of passage.value) {
      let psg;
      if (
        typeof obj[0].explicit_context === "string" &&
        obj[0].explicit_context === "c"
      )
        delete context.v;
      ...
      for (const sub_psg of psg.passages) {
        ...
        sub_psg.enclosed_absolute_indices =
          psg.type === "sequence_post_enclosed"
            ? [...psg.absolute_indices]
            : [-1, -1];
  ...
  range(passage, accum, context) {
    ...
    if (
      ...
      ((start.type === "bc" &&
        !start.passages?.[0]?.valid?.messages
    ...
    const valid = this.validate_ref(
      passage.start_context.translations,
      start_obj,
      end_obj
    );
    if (valid.valid) {
      const [return_now, return_value] = this.range_handle_valid(
        valid,
        passage,
        start,
        start_obj,
  ...
  range_handle_invalid(valid, passage, start, start_obj, end, end_obj, accum) {
    if (
      ...
      const new_end = this.range_check_new_end(
        passage.start_context.translations,
        start_obj,
        end_obj,
        valid
    ...
    if (
      ...
      const temp_value =
        end.type === "v" ? this.pluck("integer", end.value) : end.value;
      if (
        this.validate_ref(passage.start_context.translations, {
          b: start_obj.b,
          c: start_obj.c,
          v: temp_value,
        }).valid
    ...
    return this.sequence(
      passage,
      accum,
      structuredClone(passage.start_context)
    );
  }
  ...
  range_validate(valid, start_obj, end_obj, passage) {
    ...
    if (
      valid.messages?.end_verse_is_zero &&
      this.options.zero_verse_strategy !== "allow"
    )
    ...
    [start_obj.c, start_obj.v] = this.fix_start_zeroes(
      valid,
      start_obj.c,
      start_obj.v
    );
  ...
  translation_sequence(passage, accum, context) {
    ...
    for (const val of passage.value[1]) {
      const translation =
        this.books[this.pluck("translation", val).value].parsed;
      if (translation)
        translations.push({
          translation,
          system: "default",
    ...
    for (const translation of translations)
      if (this.translations.aliases[translation.translation]) {
        translation.system =
          this.translations.aliases[translation.translation].system;
        translation.osis =
  ...
  translation_sequence_apply(accum, translations) {
    ...
    for (let i = accum.length - 1; i >= 0; i--) {
      ...
      if (
        accum[i].type === "translation_sequence" ||
        accum[i].type === "stop"
      ) {
        use_i = i + 1;
        break;
    ...
    if (use_i < accum.length) {
      ...
      [, context] = this.handle_array(
        accum.slice(use_i),
        [],
        accum[use_i].start_context
      );
  ...
  pluck(type, passages) {
    for (const passage of passages)
      if (passage && passage.type && passage.type === type) return passage;
    return null;
  }
  ...
  calculate_indices(match, adjust) {
    ...
    for (let part of match.split(/[\x1e\x1f]/)) {
      ...
      if (switch_type === "book") {
        ...
        adjust =
          this.books[part_i].start_index +
          this.books[part_i].value.length -
          match_index;
        indices.push({
          start: end_index + 1,
          end: end_index + 1,
  ...
  normalize_passage_and_alternates(
    passage,
    alternates,
    adjust_end_index_by = 0
  ) {
  ...
  validate_ref(translations, start, end = null) {
    ...
    for (const translation of translations) {
      ...
      let [temp_valid] = this.validate_start_ref(
        translation.system,
        start,
        messages
      );
  ...
  validate_end_ref(system, start, end, valid, messages) {
    const order_system = this.translations.systems[system]?.order
      ? system
      : "current";
    if (end.c === 0) {
    ...
    if (end.b && this.translations.systems[order_system].order[end.b])
      valid = this.validate_known_end_book(
        system,
        order_system,
        start,
        end,
  ...
  validate_known_end_book(system, order_system, start, end, valid, messages) {
    const chapter_array =
      this.translations.systems[system]?.chapters?.[end.b] ||
      this.translations.systems.current.chapters[end.b];
    if (end.c == null && chapter_array.length === 1) end.c = 1;
    if (
      this.translations.systems[order_system].order[start.b] != null &&
      this.translations.systems[order_system].order[start.b] >
        this.translations.systems[order_system].order[end.b]
    ) {
  ...
  validate_known_start_book(system, start, messages) {
    ...
    start.c ??= 1;
    const chapter_array =
      this.translations.systems[system]?.chapters?.[start.b] ||
      this.translations.systems.current.chapters[start.b];
    if (start.c === 0) {
...
var bcv_regexps_manager = class {
  ...
  filter_books(testaments, case_sensitive) {
    ...
    this.parent.regexps.books = this.parent.regexps.all_books.reduce(
      (accum, book) => {
        let cloned_book;
  ...
  has_testament_overlap(testaments, book_testament) {
    if (
      new Set((testaments + book_testament).split("")).size <
      testaments.length + book_testament.length
    )
      return true;
    return false;
  }
  ...
  add_books(books) {
    if (books == null || !Array.isArray(books.books))
      throw new Error(
        "add_books: The argument to `add_books` should be an object with an array in `books`"
      );
    const starts = [];
    const ends = [];
    for (const pattern of books.books) {
      if (pattern == null || !(pattern.regexp instanceof RegExp))
        throw new Error(
          "add_books: The `regexp` property of each pattern should be a RegExp"
        );
      const book_data = this.get_book_testaments(pattern);
      const regexps = this.get_book_pattern_regexps(pattern, book_data);
      const regexp = new RegExp(
        regexps.pre_regexp.source +
          "(" +
          regexps.regexp.source +
          ")" +
          regexps.post_regexp.source,
        "giu"
      );
      const position =
        typeof pattern.insert_at === "string" ? pattern.insert_at : "start";
      const insert_object = {
        osis: pattern.osis,
    ...
    this.filtered_books_flags = "";
    this.filter_books(
      this.parent.options.testaments,
      this.parent.options.case_sensitive
    );
  }
  get_book_pattern_regexps(pattern, book_data) {
    ...
    for (const regexp_type of ["pre_regexp", "post_regexp"])
      if (pattern[regexp_type] == null) {
        let regexp_key =
          regexp_type === "pre_regexp" ? "pre_book" : "post_book";
      ...
      } else if (pattern[regexp_type] instanceof RegExp)
        regexps[regexp_type] = pattern[regexp_type];
      else
        throw new Error(
          "add_books: The `" +
            regexp_type +
            "` property of each pattern should be a RegExp"
        );
    return regexps;
  }
  get_book_testaments(pattern) {
    ...
    if (!Array.isArray(books))
      throw new Error(
        "add_books: The `osis` property of each pattern should be an array"
      );
    const out = {
      testament_books: {},
    ...
    for (const book of books) {
      if (
        typeof book !== "string" ||
        this.parent.translations.systems.default.order[book] == null
      )
        throw new Error("add_books: Unknown book in pattern: " + book);
      if (book in out.testament_books)
        throw new Error(
          "add_books: Every provided book should be unique. Duplicate: " + book
        );
      let testament = "o";
      if (book === "Ps") {
        ...
        const canonical_order =
          this.parent.translations.systems.default.order[book];
...
var bcv_translations_manager = class {
    ...
    if (this.parent.translations.systems[system] == null) {
      if (this.parent.options.warn_level === "warn")
        console.warn(
          "Unknown translation `" +
            new_translation +
            "` in translation_info(). Using default instead."
        );
      system = "default";
    }
  ...
  add_translations(new_translations) {
    if (
      new_translations?.translations == null ||
      !Array.isArray(new_translations.translations) ||
      new_translations.translations.length === 0
    )
      throw new Error(
        "add_translations: A `translations array in the `translations` key should have at least one object"
      );
    const normalized_translations = {};
    const texts_for_regexp = [];
    for (const translation of new_translations.translations) {
      const normalized_translation =
        this.normalize_sent_translation_data(translation);
      const insert_key = translation.text.toLowerCase();
      if (insert_key === "default" || insert_key === "current")
        throw new Error(
          "add_translations: Can't redefine `" +
            insert_key +
            "` as a translation. This built-in translation can't be redefined"
        );
      ...
      if (
        ...
        if (
          new_translations.systems != null &&
          new_translations.systems[system] != null
        )
          this.add_system(
            normalized_translation.system,
            new_translations.systems[system]
          );
        else {
          let valid_systems = Object.keys(this.parent.translations.systems);
          valid_systems = valid_systems.filter(
            (system2) => system2 !== "current"
          );
          throw new Error(
            "add_translations: Unknown translation `system`: `" +
              system +
              "`. Valid `system`s are: `" +
              Object.keys(valid_systems).join("`, `") +
              "`. You may want to check that you included this system in `systems`"
          );
        }
      else if (system === "current")
        throw new Error(
          "add_translations: Can't use `" +
            system +
            "` as a versification system for a new translation"
        );
      if (
        normalized_translations[insert_key] != null ||
        this.parent.translations.aliases[insert_key] != null
      ) {
        if (this.parent.options.warn_level === "warn")
          console.warn(
            "add_translations: `" +
              translation.text +
              "` already exists. You probably only want to do this if the old definition was wrong"
          );
  ...
  apply_case_sensitive(case_sensitive) {
    ...
    for (const [i, translation] of this.parent.regexps.translations.entries())
      this.parent.regexps.translations[i] = new RegExp(
        translation.source,
        flags
      );
  }
  normalize_sent_translation_data(translation) {
    ...
    if (text == null || typeof text !== "string" || text.length === 0)
      throw new Error(
        'add_translations: Each translation object should contain a string `text` key with a value like "KJV"'
      );
    if (text.match(/^\p{N}+$/u))
      throw new Error(
        "add_translations: A translation.text (`" +
          text +
          "`) can't be all numbers because then it would conflict with chapter and verse references."
      );
    return {
      osis:
        typeof translation.osis === "string" && translation.osis !== ""
          ? translation.osis
          : translation.text.toUpperCase(),
      system:
        typeof translation.system === "string" && translation.system.length > 0
          ? translation.system
          : "default",
    };
  }
  add_system(system, new_system) {
    if (system === "default" || system === "current")
      throw new Error(
        "add_translations: Can't use `" +
          system +
          "` as a versification system. This built-in system can't be redefined"
      );
    if (
      new_system == null ||
      (new_system.books == null && new_system.chapters == null)
    )
      throw new Error(
        "add_translations: The system object should contain `books` key, a `chapters` key or both"
      );
    ...
    if (new_system.books != null) {
      if (!Array.isArray(new_system.books) || new_system.books.length === 0)
        throw new Error(
          "add_translations: The `books` key in each `system` object should be an array with at least one string in it"
        );
      out.books = this.make_system_books(new_system.books);
    }
    if (new_system.chapters != null) {
      if (
        typeof new_system.chapters !== "object" ||
        Object.keys(new_system.chapters).length === 0
      )
        throw new Error(
          "add_translations: The `chapters` key in the each `system` object should be an object with at least one key"
        );
  ...
  make_system_books(books) {
    const all_books = structuredClone(
      this.parent.translations.systems.default.order
    );
    const new_books = {};
    let book_i = 1;
    for (const book of books) {
      if (typeof book !== "string" || all_books[book] == null)
        throw new Error(
          "add_translations: Got an unexpected OSIS value in `books` (also check for any duplicates): " +
            book
        );
    ...
    const remaining_books = Object.keys(all_books).sort(
      (a, b) => all_books[a] - all_books[b]
    );
  ...
  validate_system_chapters(chapters) {
    ...
    for (const [book, chapter_lengths] of Object.entries(chapters)) {
      ...
      if (!Array.isArray(chapter_lengths) || chapter_lengths.length == 0)
        throw new Error(
          "add_translations: Each value in `chapters` should be an array with at least one entry containing the number of verses in each chapter. Check `" +
            book +
            "`"
        );
      for (const verse_count of chapter_lengths)
        if (
          !(
            typeof verse_count === "number" &&
            verse_count >= 1 &&
            verse_count <= 200
          )
        )
          throw new Error(
            "add_translations: Unexpected value in `chapters`: " +
              verse_count +
              "`. It should be a number between 1 and 200"
          );
    }
  }
  add_new_translations_regexp(texts_for_regexp, new_translations) {
    ...
    const pre_regexp =
      new_translations?.pre_regexp instanceof RegExp
        ? new_translations?.pre_regexp
        : { source: "" };
    const post_regexp =
      new_translations?.post_regexp instanceof RegExp
        ? new_translations?.post_regexp
        : /(?![\p{L}\p{N}])/u;
    const flags = this.parent.options.case_sensitive.includes("translations")
      ? "g"
      : "gi";
    const regexp = new RegExp(
      pre_regexp.source +
...
const bcv = new bcv_parser(lang);`;
  ...
  parse(string_to_parse) {
    ...
    [string_to_parse, this.passage.books] =
      this.matcher.match_books(string_to_parse);
  ...
  parse_with_context(string_to_parse, context_string) {
    this.reset();
    [context_string, this.passage.books] = this.matcher.match_books(
      this.matcher.replace_control_characters(context_string)
    );
    ...
    string_to_parse =
      "" + (this.passage.books.length - 1) + "/9" + string_to_parse;
    [this.entities] = this.matcher.match_passages(string_to_parse);
    return this;
  }
  reset() {
  ...
  parsed_entities() {
    ...
    this.entities.forEach((entity, entity_id) => {
      ...
      if (
        (entity.type === "b" &&
          this.options.book_alone_strategy === "ignore") ||
        (entity.type === "b_range" &&
      ...
      if (entity.passages[0].translations)
        for (const translation of entity.passages[0].translations) {
          const translation_osis =
            translation.osis && translation.osis.length > 0
              ? translation.osis
              : "";
      ...
      let osises = this.parse_entity_passages(
        entity,
        entity_id,
        translations,
        system
      ...
      if (this.options.sequence_combination_strategy === "separate")
        ...
        if (
          osises[last_i].enclosed_indices &&
    ...
    entity.passages.forEach((passage, i) => {
      ...
      osises.push({
        osis: passage.valid.valid
          ? this.to_osis(passage.start, passage.end, system)
          : "",
        type: passage.type,
  ...
  to_osis(start, end, translation) {
    ...
    if (
      ...
      ((start.c === 151 && start.b === "Ps") ||
        (end.c === 151 && end.b === "Ps"))
    )
      this.fix_ps151(start, end, translation);
    const chapter_array =
      this.passage.translations.systems[translation]?.chapters[end.b] ||
      this.passage.translations.systems.current.chapters[end.b];
    ...
    if (end.v == null)
      if (
        chapter_array &&
        chapter_array[end.c - 1] &&
        existence_strategy.includes("v")
      )
        end.v = chapter_array[end.c - 1];
      else end.v = 999;
    const get_partial =
      compaction_strategy.includes("p") && (start.p != null || end.p != null);
    if (
  ...
  is_verse_consecutive(prev, check, translation) {
    ...
    const translation_order =
      this.passage.translations.systems[translation]?.order ||
      this.passage.translations.systems.current.order;
    const chapter_array =
      this.passage.translations.systems[translation]?.chapters?.[prev.b] ||
      this.passage.translations.systems.current.chapters[prev.b];
    if (prev.b === check.b) {
      ...
      return (
        prev.c === chapter_array.length && prev.v === chapter_array[prev.c - 1]
      );
    return false;
  }
  snap_range(entity, passage_i) {
    ...
    if (
      ...
      (entity.type === "sequence" &&
        entity.passages[passage_i].type === "b_range_start")
    ) {
      entity_i = 1;
      source_entity = "end";
      type = "b_range_start";
    ...
    for (const obj_type of [source_entity, target_entity])
      if (entity.passages[passage_i][obj_type].original_object != null)
        entity.passages[passage_i][obj_type] =
          entity.passages[passage_i][obj_type].original_object;
    const original_target = structuredClone(
      entity.passages[passage_i][target_entity]
    );
    entity.passages[passage_i][target_entity] = structuredClone(
      entity.passages[passage_i][source_entity]
    );
    entity.passages[passage_i][target_entity].original_object = original_target;
    entity.passages[passage_i][source_entity].original_object = structuredClone(
      entity.passages[passage_i][source_entity]
    );
  ...
  snap_sequence(type, entity, osises, i, length) {
    ...
    if (
      ...
      passage.absolute_indices[1] === entity.absolute_indices[1] &&
      i > 0
    )
...
//# sourceMappingURL=bible-passage-reference-parser_esm_bcv___parser__js.js.map
