 ...
 */
var _resolveSetOption = function _resolveSetOption(
  cfg,
  key,
  fallback,
  options
...
function createDOMPurify() {
  ...
  const _getDefaultTrustedTypesPolicy =
    function _getDefaultTrustedTypesPolicy() {
   ...
   */
  const _checkSvgNamespace = function _checkSvgNamespace(
    tagName,
    parent,
    parentTagName
  ) {
   ...
   * @returns true if a spec-compliant parser could produce this element
   */
  const _checkMathMlNamespace = function _checkMathMlNamespace(
    tagName,
    parent,
    parentTagName
  ) {
   ...
   * @returns true if a spec-compliant parser could produce this element
   */
  const _checkHtmlNamespace = function _checkHtmlNamespace(
    tagName,
    parent,
    parentTagName
  ) {
   ...
   */
  const _stripDisallowedAttributes = function _stripDisallowedAttributes(
    element
  ) {
  ...
  const _neutralizeSubtree = function _neutralizeSubtree(root) {
    ...
    while (stack.length > 0) {
      ...
      if (
        (getNodeType ? getNodeType(node) : node.nodeType) === NODE_TYPE.element
      )
   ...
   */
  const _sanitizeDisallowedNode = function _sanitizeDisallowedNode(
    currentNode,
    tagName
  ) {
  ...
  const _sanitizeElements = function _sanitizeElements(currentNode) {
    ...
    if (
      (getNodeType ? getNodeType(currentNode) : currentNode.nodeType) ===
        NODE_TYPE.element &&
   ...
   */
  const _setAttributeValue = function _setAttributeValue(
    currentNode,
    name,
    namespaceURI,
    value
  ...
  const _sanitizeShadowDOM2 = function _sanitizeShadowDOM(fragment) {
    ...
    while ((shadowNode = shadowIterator.nextNode())) {
      ...
      if (
        (getNodeType ? getNodeType(shadowNode) : shadowNode.nodeType) ===
        NODE_TYPE.element
      ) {
