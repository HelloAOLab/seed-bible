...
import { UserPresenceService } from "../../../../../packages/seed-bible-utils/application/services/UserPresenceService";
import type {
...
const makeEventPort = (): UserPresenceEventPort => ({ emit: vi.fn() });

const makeProviderPort = (
  overrides: Partial<UserPresenceProviderPort> = {}
): UserPresenceProviderPort => ({
  getCurrUserId: vi.fn().mockReturnValue("user-1"),
  getSelectedReadingInstance: vi.fn().mockReturnValue(undefined),
  getRemotesPresence: vi.fn().mockReturnValue(new Map()),
  ...overrides,
});
...
describe("updateUserPresence", () => {
  it("emits OnUserPresenceUpdate on every call", () => {
    ...
    (eventPort.emit as Mock).mockClear();
  ...
  it("includes the current user when selectedReadingInstance is provided", () => {
    ...
    const providerPort = makeProviderPort({
      getCurrUserId: vi.fn().mockReturnValue("me"),
      getSelectedReadingInstance: vi.fn().mockReturnValue(instance),
    });
  ...
  it("includes remote users from getRemotesPresence", () => {
    ...
    const providerPort = makeProviderPort({
      getRemotesPresence: vi.fn().mockReturnValue(remotes),
    });
    const svc = makeService(providerPort);
    const presence = svc.getUserPresence();
  ...
  it("replaces the previous presence map on each call", () => {
    ...
    const getSelectedReadingInstance = vi.fn().mockReturnValue(instance);
    const providerPort = makeProviderPort({
      getCurrUserId: vi.fn().mockReturnValue("me"),
  ...
  it("removes a remote user that was present before but is absent in the new update", () => {
    ...
    const getRemotesPresence = vi.fn().mockReturnValue(firstRemotes);
...
describe("getUserPresence", () => {
  ...
  it("reflects updates after updateUserPresence is called", () => {
    const getSelectedReadingInstance = vi.fn().mockReturnValue(undefined);
...
describe("getOwnUserConfigId", () => {
  it("delegates to getCurrUserId on the provider port", () => {
    const providerPort = makeProviderPort({
      getCurrUserId: vi.fn().mockReturnValue("my-config-id"),
    });
    const svc = makeService(providerPort);
  ...
  it("calls getCurrUserId each time it is invoked", () => {
    const getCurrUserId = vi.fn().mockReturnValue("user-1");
...
describe("getOwnUserPresence", () => {
  ...
  it("updates after updateUserPresence changes the current user's reading instance", () => {
    const getSelectedReadingInstance = vi
      .fn()
