import { ArrangementMapper } from "../../../../../packages/seed-bible-utils/infrastructure/mappers/ArrangementMapper";
...
describe("toTemplate — name and id generation", () => {
  ...
  it("assigns an id from generateId to the arrangement", () => {
    const { mapper } = makeMapper();
    const generateId = vi.fn().mockReturnValue("arr-id");
  ...
  it("calls generateId exactly (1 + testaments + sections + books) times", () => {
    const { mapper } = makeMapper();
    const generateId = vi.fn().mockReturnValue("x");
