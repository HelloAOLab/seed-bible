...
describe("useReadingHistoryUserFiltersSelector", () => {
  ...
  function makeScriptureMap2DContext(
  ...
  beforeEach(() => {
    ...
    (useScriptureMap2DContext as jest.Mock).mockReturnValue(
      makeScriptureMap2DContext((key) => key)
    );
  });
  ...
  it("labels the current user as 'You'", () => {
    (useScriptureMap2DContext as jest.Mock).mockReturnValue(
      makeScriptureMap2DContext((key) => (key === "you" ? "you" : key))
    );
  ...
  it("falls back to 'Guest' when profile name is empty", () => {
    (useScriptureMap2DContext as jest.Mock).mockReturnValue(
      makeScriptureMap2DContext((key) => (key === "guest" ? "guest" : key))
    );
    const filters = new Map([["user1", true]]);
