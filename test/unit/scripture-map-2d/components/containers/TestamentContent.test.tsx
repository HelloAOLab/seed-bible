...
jest.mock("scriptureMap2D.hooks.useTestamentContent", () => ({
  useTestamentContent: jest.fn(),
}));

jest.mock("scriptureMap2D.components.containers.SectionToggle", () => ({
  SectionToggle: ({
    sectionKey,
    showingContent,
  }: {
...
jest.mock("scriptureMap2D.components.ui.BooksContainer", () => ({
  BooksContainer: ({ children }: { children: preact.ComponentChildren }) => (
    <div data-testid="books-container">{children}</div>
  ),
}));

jest.mock("scriptureMap2D.components.containers.Book", () => ({
