...
} from "scriptureMap2D.components.containers.Settings";

jest.mock("scriptureMap2D.hooks.useSettings", () => ({
  useSettings: jest.fn(),
}));

jest.mock("scriptureMap2D.hooks.useClickOutside", () => ({
  useClickOutside: jest.fn(),
}));

jest.mock("scriptureMap2D.components.containers.ProjectStateSetter", () => ({
  ProjectStateSetter: () => <div data-testid="project-state-setter" />,
}));

jest.mock(
  "scriptureMap2D.components.containers.ProjectFiltersSelector",
  () => ({
...
jest.mock(
  "scriptureMap2D.components.containers.ReadingHistoryUserFiltersSelector",
  () => ({
...
jest.mock(
  "scriptureMap2D.components.containers.ReadingHistoryTimeline",
  () => ({
    ReadingHistoryTimeline: () => (
      <div data-testid="reading-history-timeline" />
    ),
  })
);
...
function makeHookResult(overrides: Record<string, unknown> = {}) {
  return {
    settingsClass: "scripture-map-2d-settings",
...
describe("Settings", () => {
  ...
  describe("structure", () => {
    it("applies settingsClass to the root element", () => {
      setup({ settingsClass: "scripture-map-2d-settings collapsed" });
      expect(
        container.querySelector(".scripture-map-2d-settings.collapsed")
      ).not.toBeNull();
    });
  ...
  describe("Project mode section", () => {
    ...
    it("does not render ProjectStateSetter when mode is Viewer", () => {
      setup({ mode: ScriptureMap2DModes.Viewer, project: { name: "Test" } });
      expect(
        container.querySelector("[data-testid='project-state-setter']")
      ).toBeNull();
    });
    ...
    it("does not render ProjectStateSetter when project is undefined", () => {
      setup({ mode: ScriptureMap2DModes.Project, project: undefined });
      expect(
        container.querySelector("[data-testid='project-state-setter']")
      ).toBeNull();
    });
