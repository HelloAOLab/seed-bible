...
describe("BibleReaderToolbar — chat-first mobile tab", () => {
  ...
  // "You" (the Profile entry point, #1554) sits second, so Bookmarks is the
  // fifth tab rather than the fourth. It is still a bottom tab, not demoted
  // into More — that is what the Bookmarks assertion below pins down.
  it("keeps Bookmarks as a bottom tab when chatFirst is not set", async () => {
    await renderToolbar();

    expect(tabButton("Bookmarks")).not.toBeNull();
    expect(tabButton("Chat")).toBeNull();
    expect(mobileTabLabels().slice(0, 5)).toEqual([
      "Today",
      "You",
      "Search",
      "Bible",
