...
beforeEach(() => {
  (globalThis as any).configBot = { id: "config-bot-id" };
});
...
describe("getCurrUserId", () => {
  it("returns configBot.id", () => {
    expect(makeProvider().getCurrUserId()).toBe("config-bot-id");
  });

  it("reflects changes to configBot.id at call time", () => {
    (globalThis as any).configBot = { id: "updated-id" };
    expect(makeProvider().getCurrUserId()).toBe("updated-id");
  });
});
