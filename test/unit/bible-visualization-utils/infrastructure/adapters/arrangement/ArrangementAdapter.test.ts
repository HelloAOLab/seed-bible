...
const makeMapperPort = (
  toDomainFn: jest.Mock = jest.fn().mockReturnValue(makeSectionInfo())
): SectionInfoMapperPort => ({ toDomain: toDomainFn });
...
describe("toDomain — mapper delegation", () => {
  it("calls toDomain on the mapper for each section", () => {
    const toDomainFn = jest.fn().mockReturnValue(makeSectionInfo());
    const adapter = makeAdapter(makeMapperPort(toDomainFn));
    adapter.toDomain(
