...
export async function uploadExtensionAux(
  ...
  const uploadCommand = `casualos ${globalOpts} upload-package --record "${uploadRecordName}" --address "${meta.name}" --key "minor" --file "${filePath}" --markers "publicRead" --entitlements "data:personal file:personal"`;
