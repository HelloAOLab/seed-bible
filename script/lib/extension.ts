...
export async function uploadExtensionAux(
  ...
  let uploadCommand = `casualos --endpoint "${"https://api.ao.bot"}" upload-package --record "${recordKey ?? uploadRecordName}" --address "${meta.name}" --file "${filePath}" --markers "publicRead" --entitlements "data:personal file:personal" -v "minor"`;
  if (sessionKey) {
