...
**Unused CSS**: production builds run PurgeCSS (`script/lib/purgeCss.ts`) over both the emitted stylesheets and the inlined critical CSS, dropping any rule whose selector appears in no source file or built JS chunk. Class names the app assembles at runtime are handled automatically as long as the literal prefix is visible in the code — `` `sb-extension-state-${state}` `` keeps every `.sb-extension-state-*` rule. A single-segment prefix (`` `sb-${id}` ``) is too broad to derive from and needs an explicit entry in `PURGE_SAFELIST`, which is also where theming and highlight selectors are pinned. If a style goes missing only in a production build, confirm it with `DISABLE_PURGECSS=true pnpm build` before looking elsewhere.
