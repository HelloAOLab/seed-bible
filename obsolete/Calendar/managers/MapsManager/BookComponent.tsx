...
export const Book = ({
  ...
  const chaptersCount = useMemo(() => {
    return StacksManager.tags.booksStaticInfo[mapPanelBookInfo.name]
      .numberOfChapters;
  }, []);
  ...
  const bookCoverHeight = useMemo(() => {
    return `${Math.round(thisBot.GetMapBookHeightByName({ bookName: mapPanelBookInfo.name }) * scaleFactor)}px`;
  }, [scaleFactor]);
  const isLastBookInSection = useMemo(() => {
    return bookIndex == section.books.length - 1;
  }, []);
  const isOnlyBook = useMemo(() => {
    return isFirstBookInSection && isLastBookInSection;
  }, []);
  const isMiddleBook = useMemo(() => {
    return (
      (!isFirstBookInSection || !isFirstSectionInTestament) &&
      (!isLastBookInSection || !isLastSectionInTestament)
    );
  }, []);
  ...
  return (
    <div
      className={`mapBookContainer${hidden ? " hidden" : ""}${showingSection ? "" : " collapsed"}${isLastSectionInTestament ? " lastSection" : ""}${isFirstSectionInTestament ? " firstSection" : ""}${isFirstBookInSection ? " firstBook" : ""}${isLastBookInSection ? " lastBook" : ""}${isOnlyBook ? " onlyBook" : ""}${isMiddleBook ? " middleBook" : ""}`}
    >
      {showLabels && (
        <div
          ...
          onClick={() => {
            if (isFirstBookInSection) toggleShowSection(section);
          }}
          style={{
            borderColor: isFirstBookInSection && section.color,
            backgroundColor:
              isFirstBookInSection &&
              (sectionNameHovered ? "#FFFFFF" : section.color),
          }}
        >
          <span
            style={{
          ...
          >
            {showChapters &&
              [...Array(chaptersCount)].map((_, index) => {
                return (
                  <Chapter bookName={mapPanelBookInfo.name} index={index} />
                );
              })}
          </div>
        </div>
