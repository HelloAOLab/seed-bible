...
const originalMapData = thisBot.GetMapDataById({
  mapId: mapChapterData.originalMapId,
});
  ...
  case StackElementInteractionType.Click:
    {
      if (!thisBot.masks.isAnimatingMap) {
        if (InstanceManager.masks.isHighlightToolEnabled) {
          ...
          if (
            !mapChapter.masks.isSelecting &&
            !mapChapter.masks.isDeselecting
          ) {
  ...
  case StackElementInteractionType.Drop:
    {
      ...
      if (mapChapterData.isSelected) {
        if (
          ...
          originalMapData.isChapterExpandEnabled
        ) {
          thisBot
            .DeselectChapter({ mapChapterData, mapData: originalMapData })
            .then(() => {
              thisBot.TrySelectChapter({
                mapChapterData,
                mapData: originalMapData,
