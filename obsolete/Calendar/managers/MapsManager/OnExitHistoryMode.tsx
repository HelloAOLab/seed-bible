...
elementsData.forEach((elementData) => {
  const isElementAvailable =
    ...
    (elementData instanceof MapBookData ? !elementData.isSelected : true);

  if (isElementAvailable) {
    if (elementData instanceof MapChapterData && elementData.isSelected) {
      const mapData = thisBot.GetMapDataById({
        mapId: elementData.parentDataIds.mapId,
      });
      setTagMask(
        elementData.element,
        "color",
        elementData.isSelected && !elementData.element.masks.isExpanded
          ? mapData.chapterSelectColor
          : (elementData.highlightColor ??
              elementData.element.tags.initialColor)
      );
          ...
          if (chunk.masks.isSelected) {
            if (
              ...
              chunk.vars.verses.forEach((verse) => {
                let verseHighlightInfo = elementData.GetHighlightInfoByKey(
            ...
            let chunkHighlightInfo = elementData.GetHighlightInfoByKey(
      ...
      setTagMask(
        elementData.element,
        "color",
        elementData.highlightColor ?? elementData.element.tags.initialColor
      );
    }
  }
});
