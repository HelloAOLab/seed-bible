...
export const MapWrapper = () => {
  ...
  const getFixedSizeByRatio = useCallback(
    (ratio) => {
  ...
  const FIXED_SIZE_2 = useMemo(() => {
    return getFixedSizeByRatio(SIZE_RATIO_2);
  }, [scaleFactor]);
  const FIXED_SIZE_4 = useMemo(() => {
    return getFixedSizeByRatio(SIZE_RATIO_4);
  }, [scaleFactor]);
  const FIXED_SIZE_8 = useMemo(() => {
    return getFixedSizeByRatio(SIZE_RATIO_8);
  }, [scaleFactor]);
  const FIXED_SIZE_12 = useMemo(() => {
    return getFixedSizeByRatio(SIZE_RATIO_12);
  }, [scaleFactor]);
  const FIXED_SIZE_20 = useMemo(() => {
    return getFixedSizeByRatio(SIZE_RATIO_20);
  }, [scaleFactor]);
  const bookWidth = useMemo(() => {
    return getFixedSizeByRatio(MapElementMeasurements.BookScaleX);
  }, [scaleFactor]);
  const chapterGap = useMemo(() => {
    return getFixedSizeByRatio(MapElementMeasurements.ChapterGap);
  }, [scaleFactor]);
  const chapterWidth = useMemo(() => {
    return getFixedSizeByRatio(MapElementMeasurements.ChapterWidth);
  }, [scaleFactor]);
  const chapterHeight = useMemo(() => {
    return getFixedSizeByRatio(MapElementMeasurements.ChapterHeight);
  }, [scaleFactor]);
