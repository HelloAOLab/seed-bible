...
describe("toDomain", () => {
  it("maps bot.id to id", () => {
    expect(new InfoLabelTextMapper().toDomain(makeBot()).id).toBe("bot-1");
  });

  it("maps a different bot.id", () => {
    expect(new InfoLabelTextMapper().toDomain(makeBot({ id: "x-99" })).id).toBe(
      "x-99"
    );
  });

  it("sets type to 'InfoLabelText'", () => {
    expect(new InfoLabelTextMapper().toDomain(makeBot()).type).toBe(
      "InfoLabelText"
    );
  });

  it("returns an object with exactly id and type", () => {
    const result = new InfoLabelTextMapper().toDomain(makeBot({ id: "abc" }));
    expect(result).toEqual({ id: "abc", type: "InfoLabelText" });
  });

  it("produces distinct objects for distinct bots", () => {
    const a = new InfoLabelTextMapper().toDomain(makeBot({ id: "a" }));
    const b = new InfoLabelTextMapper().toDomain(makeBot({ id: "b" }));
    expect(a).not.toBe(b);
    expect(a.id).toBe("a");
    expect(b.id).toBe("b");
  });
  ...
  it("does not include extra tags in the result", () => {
    const result = new InfoLabelTextMapper().toDomain(makeBot()) as any;
...
describe("toInfrastructure", () => {
  it("calls byID with the piece id", () => {
    new InfoLabelTextMapper().toInfrastructure(makePiece({ id: "p-1" }));
  ...
  it("calls getBot with the result of byID", () => {
    ...
    new InfoLabelTextMapper().toInfrastructure(makePiece());
  ...
  it("returns the bot when getBot finds it", () => {
    ...
    expect(new InfoLabelTextMapper().toInfrastructure(makePiece())).toBe(bot);
  });
