...
while (masks["run"]) {
  ...
  for (let key in toEraseBots) {
    distance = Vector3.distanceBetween(
      new Vector3(
        toEraseBots[key].tags[dim + "X"],
        toEraseBots[key].tags[dim + "Y"],
        toEraseBots[key].tags[dim + "Z"]
      ),
      new Vector3(
        thisBot.tags[dim + "X"],
        thisBot.tags[dim + "Y"],
    ...
    if (distance <= thisBot.tags.scaleX) {
      if (
        toEraseBots[key].tags.isBibleElement ||
        toEraseBots[key].tags.isBibleTransformer ||
        toEraseBots[key].tags.isSectionShadow
      ) {
