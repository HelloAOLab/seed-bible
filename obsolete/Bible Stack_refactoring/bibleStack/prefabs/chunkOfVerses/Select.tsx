...
verses.forEach((verse) => {
  const positionX =
    chunkPosition.x -
    chunkScales.x / 2 +
    fixedVerseScales.x / 2 +
    gapBetweenVerses / 2 +
    column * (fixedVerseScales.x + gapBetweenVerses);
  const positionY =
    chunkPosition.y +
    chunkScales.y / 2 -
    fixedVerseScales.y / 2 -
    gapBetweenVerses / 2 -
    row * (fixedVerseScales.y + gapBetweenVerses);
  ...
  setTagMask(
    verse,
    "versePath",
    `${thisBot.masks.parentBookName} ${thisBot.masks.chapterNumber} ${currentChapterNumber}`
  );
  ...
  if (BibleVizUtils.Data.masks.isInHistoryMode)
    ...
    const chapterData = thisBot.masks.chapterDataId
      ? BibleStackManager.GetChapterDataById({
    ...
    if (currentHighlightInfo)
      setTagMask(verse, "color", currentHighlightInfo.color);
  }
