...
colorBot.masks.difference = [
  (colorBot.masks.startColor[0] - colorBot.masks.endColor[0]) / 40,
  (colorBot.masks.startColor[1] - colorBot.masks.endColor[1]) / 40,
  (colorBot.masks.startColor[2] - colorBot.masks.endColor[2]) / 15,
];
colorBot.masks.forward = true;
let interval = setInterval(() => {
  if (colorBot.masks.forward) {
    colorBot.masks.currentColor = [
      colorBot.masks.currentColor[0] - colorBot.masks.difference[0],
      colorBot.masks.currentColor[1] - colorBot.masks.difference[1],
      colorBot.masks.currentColor[2] - colorBot.masks.difference[2],
    ];
    if (colorBot.masks.currentColor[0] < colorBot.masks.endColor[0]) {
      colorBot.masks.forward = false;
    }
    if (
      colorBot.tags.id === tags.currentWritingBotId ||
      (colorBot.tags.textBot
        ? colorBot.tags.textBot === tags.currentWritingBotId
        : false)
    ) {
    ...
    setTagMask(
      colorBot,
      "strokeColor",
      `rgb(${Math.floor(colorBot.masks.currentColor[0])},${Math.floor(colorBot.masks.currentColor[1])},${Math.floor(colorBot.masks.currentColor[2])})`,
      "tempLocal"
    );
  } else {
    colorBot.masks.currentColor = [
