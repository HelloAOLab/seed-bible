...
if (that?.bot.id !== tags.id && that?.bot.tags.textBox) {
  let firstBot = whisper(typingTool, "createMMBot", {
    from: { x: tags[dim + "X"], y: tags[dim + "Y"] },
    label: thisBot.masks.label ? thisBot.masks.label : " ",
    config: {
      formAddress: thisBot.masks.formAddress
        ? thisBot.masks.formAddress
        : thisBot.tags.formAddress
          ? thisBot.tags.formAddress
          : null,
      voiceNote: thisBot.masks.voiceNote
        ? thisBot.masks.voiceNote
        : thisBot.tags.voiceNote
          ? thisBot.tags.voiceNote
          : null,
  ...
  let secondBot = whisper(typingTool, "createMMBot", {
    from: { x: tags[dim + "X"] + 10, y: tags[dim + "Y"] },
    parentBot: firstBot[0].bot,
