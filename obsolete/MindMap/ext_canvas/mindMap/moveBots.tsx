...
for (let i = 0; i < that.childrenIds.length; i++) {
  ...
  children.tags.initPos = {
    x: children.tags[dim + "X"] - that.xDisposition,
    y: children.tags[dim + "Y"] - that.yDisposition,
  };
  let childNewPos = {
    x: children.tags[dim + "X"] - that.xDisposition,
    y: children.tags[dim + "Y"] - that.yDisposition,
  };
  let childIndexNewPos = {
    x: childernIndex.tags[dim + "X"] - that.xDisposition,
    y: childernIndex.tags[dim + "Y"] - that.yDisposition,
  };
