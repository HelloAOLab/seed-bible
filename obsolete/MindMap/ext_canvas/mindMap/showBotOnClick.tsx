...
if (masks.currentState) {
  // need to hide the bots
  ...
  // need to show the bots
  setTagMask(thisBot, "color", "#FF5252", "shared");
  let x = 0;
  for (let i = 0; i < controlBot.masks.childIds.length; i++) {
    if (
      getBot("mmTypingManager").tags.focusManager.childIds.indexOf(
        controlBot.masks.childIds[i]
      ) === -1
    ) {
      let subBot = getBot(byTag("id", controlBot.masks.childIds[i]));
      let subIndexBot = getBot(byTag("id", subBot.tags.indexBot));
      if (
        controlBot.masks.lineTo.indexOf(controlBot.masks.childIds[i]) === -1
      ) {
        setTagMask(
          controlBot,
          "lineTo",
          [...controlBot.masks.lineTo, controlBot.masks.childIds[i]],
          "shared"
        );
      } else {
        continue;
