  ...
  if (subBot) {
    ...
    if (subBotChildrens.length === 1) {
      ...
      whisper(typingTool, "makeTextBox", {
        x: subBotChild.masks[dim + "X"] - 0.5,
        y: subBotChild.masks[dim + "Y"],
        label: subBotChild.masks.label,
      });
      destroy(subBotChild);
    } else {
...
let childIds = controlBotParent.masks.childIds
  ? [...controlBotParent.masks.childIds]
  : [];
